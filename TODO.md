...
- 要新寫的判斷方法（2026-09-24 審查 web app 原型時討論出來）：
  ...
  - 上層的需求要動到底層的資料或規則時，先問「把上層拿掉，這個改動還有沒有意義」：
    - 有意義 → 改底層，底層文件用自己的理由寫，不提「因為某個上層需要」。例：職缺的年資要求加進職缺欄位契約、`地區` 拆成縣市行政區與街道地址、「改版時資料不遺失」寫成 job-database 的非功能需求
    - 沒意義 → 上層自己加，寫成上層自己的故事。例：`評分明細`、`供應商`、`模型`，即使和底層的資料放在同一張表
    - 上層的資料要有同樣的保證時（例如評分紀錄改版時不遺失），由上層為自己的資料另外寫，底層不替上層保證
  - 上層推翻底層既有的規則要盡量避免：讀者要讀兩份文件才知道實際行為。優先改底層或只做新增；推翻的地方一多，代表邊界切錯了
    - 例：history 故事推翻評分資料庫的「寫入時覆寫」「清掉評分即可重評」，最後發現評分資料庫本來就不該獨立成功能
  - 拆分或新增功能時的檢查順序：這個功能單獨存在時解決哪個使用者問題 → 沒有上層時最少要有什麼 → 依賴方向是否單向
- 要搬動與調整的：
  - overview 的依賴方向規則：判斷方法搬到新文件，overview 只留功能的定義與連結
  - documentation.md「功能漸進疊加」那一項：設計判斷搬到新文件，只留寫法（上層的故事寫明疊加了什麼、底層哪些規則不變、底層的驗收只驗底層定義的內容），並連到新文件
  - documentation.md 現在寫「擴充包括……改動底層的規則」，一律由上層處理，要改成先依上面的問題判斷，和新文件一致
  - [功能文件範本](docs/product/features/_feature_template.md)開頭的註解提到「漸進疊加」見 documentation.md，改連新文件
  - CLAUDE.md 的文件索引、[docs/README.md](docs/README.md) 的 conventions 分區各加一行
  - 新文件寫好後，Claude 記憶裡的 incremental-feature-design 改成只指向這份文件
- 開工時要決定：
  - 放在 `docs/conventions/`（規劃時的慣例）還是 `docs/product/`（產品規劃的方法）；目前傾向 conventions，因為它和 documentation.md 一樣是跨功能、跨文件的規則
  - 要不要寫一份 `docs/conventions/decisions/` 的決策紀錄，說明為什麼從 documentation.md 拆出來
