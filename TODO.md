# TODO

尚未要做、但已經確認之後要處理的事項。開始做某項時，從這裡移除，並依 CLAUDE.md 的功能開發流程進行。

## 實際使用前

- [ ] `profile/preferences.yaml` 與 `profile/experience.md` 目前是測試用的虛構資料，執行 job-auto-scoring AC-score-real（真實評分）前要換成自己的偏好與經歷，才能判斷 AI 的理由是否與自己的判斷相符

## web app

介面形式與階段順序見 [決策紀錄：介面形式選型](docs/product/decisions/interface-selection.md)，以下是第 2 階段之後的項目，各自開工時才寫功能文件。

- [ ] 唯讀的職缺瀏覽頁（第 2 階段）：表格（排序、篩選）加詳情頁（完整工作內容、四個維度分數、評語），取代把結果檔匯出到表格工具
  - 技術選型暫定 FastAPI + Jinja2 + HTMX：伺服器端渲染、無前端 build 步驟。替代方案是 Streamlit，做得快但每次互動整頁重跑，上百筆職缺的逐筆判讀會卡、也無法做鍵盤操作。開工時把這個取捨寫成 `docs/tech/decisions/` 的技術決策紀錄
- [ ] 職缺查詢的篩選條件（關鍵字、縣市）：查詢介面目前只能依寫入的執行編號篩選，瀏覽頁有實際需求時再加
- [ ] 標記（第 3 階段）：已讀、想投、略過與備註，要新增一張表。這是 UP-05 唯一的資料來源，也能就地標記抽查到的誤殺
- [ ] 手動填分的入口：job-score-database 的 manual 故事只提供寫入的方式，CLI 或瀏覽頁上的填分表單等瀏覽頁開工時再決定
- [ ] 趨勢圖表（第 4 階段）：trend-analysis 的交付形式等瀏覽頁做完再定，現在不寫進功能清單
- [ ] 從介面觸發抓取與評分（第 5 階段）：要處理背景作業、進度回報、失敗狀態與重試
- [ ] notebook 的去留：`notebooks/analyze_104_jobs.ipynb` 目前讀 `output/104/` 的 JSON 並自己重做一次去重，等瀏覽頁做完再決定改成讀資料庫還是退場

## 評分維度（job-auto-scoring 後續迭代）

- [ ] 通勤便利度：候選做法有依行政區分級（加上捷運標籤加分），或用地圖 API 計算實際通勤時間
- [ ] 資歷門檻：要先擴充 104-job-scraper，輸出年資要求
- [ ] 工作型態與福利（遠端、彈性工時等）：遠端資訊要先擴充爬蟲才拿得到
- [ ] 競爭程度與新鮮度（應徵人數、更新日期）：比較適合當參考資訊或篩選條件，不算進總分

## 爬蟲

- [ ] 擴充爬蟲欄位（年資要求、遠端等工作型態），供上方「資歷門檻」「工作型態與福利」維度使用

## 職缺自動評分（範圍外）

- [ ] 評分結果重試機制：單筆 AI 呼叫失敗時自動重打，目前單筆失敗會跳過該筆繼續，不重試
- [ ] 評分 CLI 改從職缺資料庫讀取職缺（例如 `--run-id`），目前讀爬蟲輸出的 JSON
- [ ] 記錄評分使用的設定：評分紀錄目前只存供應商與模型，看不出當時的偏好檔與經歷內容。要比較不同設定的結果時，再加一張設定表（存偏好檔與經歷全文），每筆自動評分紀錄記下它用的設定
- [ ] `--no-cache` 強制重新呼叫 AI：目前要重問 AI 只能改動快取鍵的來源（模型、提示詞模板、經歷、`目標方向`、`產業偏好`），或刪掉評分紀錄；job-auto-scoring 的 history 故事完成後不再提供刪除，只剩改動快取鍵的來源

## 評分資料庫

- [ ] 既有資料庫的 `job_scores` 遷移：`open_db` 只用 `CREATE TABLE IF NOT EXISTS`，不會修改已經存在的表。在 manual 故事之前建立的資料庫，`評分結果` 仍是 `NOT NULL`，手動評分寫入時會丟出 `IntegrityError`；自動評分淘汰時寫入的舊列，`評語` 也還是 `NULL`。要補遷移：偵測舊的 schema 後重建 `job_scores` 並保留原有資料

## 職缺資料庫

- [ ] 寫入職缺時去掉 `職缺代碼` 的前後空白：前後有空白的代碼存進 `jobs` 後，和評分紀錄 JOIN 不到。評分資料庫的手動評分維持原樣寫入，不在評分端處理
- [ ] 職缺欄位契約與執行紀錄的平台中立化：目前的欄位取自 104 職缺，執行紀錄的條件欄位（`關鍵字`、`地區`、`職缺性質`、`頁數`、`來源檔`）也是 104 留下的形狀。接第二個求職平台時要處理跨平台的職缺識別（現在的 `職缺代碼` 就是 104 的 `jobNo`）、來源平台欄位，以及各平台特有欄位怎麼放

## 文件

- [ ] 在 [documentation.md](docs/conventions/documentation.md#功能文件的結構) 補上「範圍外」要列哪些項目：實作者合理會以為包含、但其實不做的事（屬於相鄰功能、順手會做的下一步、刻意不採用的做法），不是列出所有沒做的事。已寫在 docs/product/overview.md「非目標」的事不重複列
