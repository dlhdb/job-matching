...
- [ ] F4 規劃時，訂定第二個產品目標「了解不同產業、公司現在的發展方向」的成功指標（見 [docs/README.md](docs/README.md#成功指標)）
...
- [ ] F2-01、F2-02、F5-01 改成使用者故事結構（見 [conventions.md](docs/conventions.md#功能文件的結構)），並補上「非功能需求」一章：目前只有 F1-01 遷移過，這三份還是「需求／設計／驗收」各一章、FR／AC 用流水號，AC 標題也還列著「涵蓋 FR-…」。F5-01 引用 F2-02 的 `FR-1`，遷移時一併改成描述加章節連結。等 F1-01 的新結構實際用過一輪、確認好讀再遷
