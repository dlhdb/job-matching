...
需要使用者做決定時，依問題的複雜度選擇詢問方式（包括功能開發與 commit 前審查時的詢問）：

- 可以簡單回答的問題（選項少、不需要太多脈絡說明）：直接在對話中問。
- 複雜的問題（需要較多脈絡說明，或題數多）：改寫成問卷放在 `to-be-confirm/<主題>.md`，使用者填完後會通知你。
...
4. commit 前先審查修改：
   ...
   - 修正發現的問題並重跑測試，有改動就重新暫存，再執行一次 `/code-review medium`，直到除了已向使用者說明、決定不修的問題以外，沒有發現其他問題為止；不修的問題向使用者說明原因。
