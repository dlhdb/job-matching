...
- 功能之間的資料流見 [docs/README.md 的使用者旅程](README.md#使用者旅程)。
- 各模組的細節見對應的功能文件，本文件只列總覽並連過去。
- 〔規劃中〕標示尚未實作的部分。

## 執行方式

- 在本機手動執行，不排程（見 [非目標](README.md#非目標)）。
- 每個進入點都是 `src/` 下的一支腳本，以 `uv run src/<腳本>.py` 執行：
  - `fetch_104_jobs.py`：抓取 104 職缺（見 [104-job-scraper 的 CLI](features/104-job-scraper.md#822-cli)）
  - `import_jobs.py`：把既有的職缺 JSON 匯入資料庫（見 [job-database 的匯入 CLI](features/job-database.md#522-匯入-cli)）
  - `score_job.py`：評分單筆或整批職缺（見 [job-scoring 的 CLI](features/job-scoring.md#1124-cli)）
  - `mcp_server.py`：〔規劃中〕讓 agent 以 tool 操作抓取、評分與查詢（見 [mcp-server 的架構](features/mcp-server.md#921-架構)）
...
`data/jobs.db`：所有功能共用的 SQLite 資料庫，不進版控。選用 SQLite 的理由見 [決策紀錄 0005](decisions/0005-database-selection.md)。
...
- LLM：Gemini，使用 `google-genai`
  - 經由 `LLMClient` 抽象層呼叫，換供應商不必改其他模組（見 [LLM 供應商抽象層](features/job-scoring.md#1123-llm-供應商抽象層)）
- 資料驗證：Pydantic，驗證偏好檔、AI 輸出與評分結果
- 設定檔：偏好檔用 YAML（`pyyaml`），API key 用 `.env`（`python-dotenv`）
- Agent 介面：〔規劃中〕官方 `mcp` Python SDK，以 stdio 傳輸（見 [mcp-server 的架構](features/mcp-server.md#921-架構)）
