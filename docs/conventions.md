...
骨架見 [feature_template.md](features/feature_template.md)。以下四條是骨架背後的規則：

- **系統能力是第二層**：一個系統能力一章，章內固定是需求／設計／驗收三小節。同一個系統能力的需求、規則與驗收要放在一起，不要按「需求」「設計」「驗收」各切一章再用連結互指。功能只有一個系統能力時也保持同樣的結構，所有功能文件形狀一致。
- **系統能力用它做的事命名**，不直接複製功能名稱。
- **兩個以上系統能力用到才上提到「共用設計」**，只有一個系統能力用到的就留在那個系統能力裡。
- **〔規劃中〕標在系統能力層級**：寫在該系統能力章節的第一行，不寫進標題（標題文字決定錨點，狀態改變時錨點才不會跟著變），該系統能力內部不再逐條標記。被新系統能力改動到的既有系統能力或共用章節，在被改到的那一行標〔規劃中〕，因為這些章節的其餘部分已經完成。

FR 與 AC 的 ID 用英文簡短描述，不用流水號：`FR-<系統能力 slug>-<重點>`、`AC-<系統能力 slug>-<重點>`。AC 驗的就是同名 FR 時沿用同一個重點名稱。系統能力只有一條需求時，可以省略 `-<重點>`。這樣讀到 ID 就知道它屬於哪個系統能力、在講什麼，插入新條目也不必重編既有編號。
