...
- 評分資料庫的程式碼放在 `job_db` 套件，不另開套件：同一個套件管理整份資料庫的連線與 schema。
...
- `總分`（INTEGER | null）
- `評語`（TEXT）：必填，由寫入的函式檢查
  - 欄位不設 `NOT NULL`：改為必填前寫入的舊資料可能是 `NULL`，舊資料庫的遷移見 [待辦：補上評分紀錄的遷移](../../../backlog/tasks/score-history-migration.md)
- 其他功能疊加的欄位見各功能的技術設計
...
- 手動評分寫入新的一列，不動既有的列；同一筆職缺有多列時，查詢以哪一列為準見各功能的技術設計。
  - 這次沒寫到的欄位是 `NULL`，本功能不必知道其他功能疊加了哪些欄位。
