...
- **輸入與輸出**
  ...
  - 輸出：`output/104/` 下同名的 CSV 與 JSON（見 [§5.4](#54-輸出)），欄位依 [§5.5](#55-欄位字典) 的欄位字典。JSON 給 job-scoring 評分，也給 [job-database](job-database.md) 匯入資料庫；CSV 供人用 Excel 瀏覽。
  - 抓完後預設由 job-database 把同一批職缺寫進 `data/jobs.db`（見 [job-database §5.5](job-database.md#55-cli)）。
...
寫入職缺資料庫的 `--db`、`--no-db` 見 [job-database §5.5](job-database.md#55-cli)。
