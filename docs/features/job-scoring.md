...
- 輸入的個人資料：`profile/preferences.yaml` 與 `profile/experience.md`，格式見 [§4.2.1](#421-個人資料檔)。
  - 由使用者自己維護，不來自其他功能。
- 〔規劃中〕輸入的上次評分：資料庫中同一筆職缺上次的 AI 維度與評語，用於沿用（見 [§8.2.1](#821-快取鍵)）。
...
- 〔規劃中〕評分前先查上次的評分（見 [§8](#8-重跑時不重複付-ai-費用cache)）。
- 依輸入順序逐筆處理，不平行呼叫 AI。
...
- 評分結果保存在 `--db` 指定的資料庫（預設 `data/jobs.db`，見 [§11.2.2](#1122-cli)），可以和職缺資料一起以 `職缺代碼` 查詢。
- 每筆職缺一筆紀錄，只保留最新一次成功的評分：
  - 評分時間
  - 是否淘汰、總分、評語：可以直接用 SQL 篩選與排序
...
〔規劃中〕查上次的評分與沿用尚未實作：目前沒被淘汰的職缺一律呼叫 AI。
...
- FR-cli：提供 `src/score_job.py` CLI，參數與錯誤訊息依 [§11.2.2](#1122-cli)。
  ...
  - `--dry-run` 必須搭配 `--job-no`。
- FR-llm：AI 供應商可以替換，規則依 [§11.2.3](#1123-ai-供應商與-api-key)。
  - 預設使用 Gemini。
  - 換供應商時不必修改評分規則的程式。
...
- API key 從環境變數 `GEMINI_API_KEY` 讀取：
  ...
  - 只有真的要呼叫 AI 時才需要（被淘汰、`--dry-run` 不需要）。
...
- When：
  ...
  - (b) 指定不存在的職缺代碼
  - (c) 指定不支援的供應商
  - (d) 檢查評分規則的程式依賴哪些 AI 供應商的套件
- Then：
  - (a) 顯示提到 `GEMINI_API_KEY` 的錯誤訊息並結束
  - (b) 顯示錯誤訊息並結束
  - (c) 被拒絕，提示不支援的供應商
  - (d) 只有 AI 供應商的轉接層依賴 Gemini 的套件
- 通過條件：全部符合。
