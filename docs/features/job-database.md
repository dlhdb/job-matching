...
[§6 非功能需求](#6-非功能需求)與 [§7 共用規則](#7-共用規則)不是使用者故事。§7 放跨越各章的規則：資料庫路徑與保存的資訊。
...
- 只有 `t` 不早於資料庫中的最後出現時間時，才用新資料覆蓋欄位。結果：
  - 匯入舊檔案的先後順序不會影響結果。
  - 資料庫永遠保留最新一版的內容。
- 每筆職缺都記為這次執行出現的職缺，不管是新增還是更新。

#### 4.2.2 詳情欄位不被 null 覆蓋

`工作內容`、`薪資待遇` 保留舊值，是缺值不回填（見 [§6](#6-非功能需求)）唯一的例外，理由如下：

- 爬蟲取 104 職缺頁面的內容常因暫時性錯誤失敗，失敗時這兩欄為 `null`。
- 資料庫中的舊值來自同一個來源的同一個欄位，不是用搜尋摘要之類的其他來源回填。
...
- When：分別以 `--db <測試用路徑>`、`--db <測試用路徑> --no-db` 執行爬蟲
- Then：
  - 只有 `--db`：資料庫中有這次抓到的職缺，並有一筆 `來源` 為 `爬蟲` 的執行紀錄
  - 加上 `--no-db`：不建立資料庫檔
  - 兩者都照常產生 CSV／JSON
- 通過條件：全部符合。
...
- 每個檔案是一次執行，各自全部寫入或完全不寫入。
- 寫入後顯示的摘要同 [§4.2.3](#423-爬蟲的寫入參數)。
...
- 爬蟲本身寫入資料庫時，不記錄來源檔。之後再匯入同一個 JSON：
  - 會被當成另一次執行，多一筆執行紀錄。
  - 職缺資料不變：兩者的 `t` 相同、職缺內容也相同，依 [§4.2.1](#421-寫入規則) 的規則不會改變。
...
- 顯示 `[-]` 並以錯誤結束：沒有指定檔案，或所有檔案都因為錯誤而被略過。
...
- 104-job-scraper 欄位字典的全部欄位，順序與字典相同，以 `職缺代碼` 識別
- `首次出現時間`：這筆職缺第一次被抓到的時間
- `最後出現時間`：這筆職缺最後一次被抓到的時間
- 不記錄下架狀態：「最後出現時間」較早，不代表職缺已經下架，可能只是之後的搜尋條件沒有涵蓋它。

每次執行（抓取或匯入）：

- `執行編號`：自動遞增的編號
- `執行時間`：爬蟲寫出 CSV／JSON 時的時間，與檔名中的時間戳相同
...
- `地區`：縣市名稱，全台灣時為 `全台灣`
  - 匯入時為 `null`
- `職缺性質`：`0`／`1`／`2`，意義同 [104-job-scraper 的 CLI](104-job-scraper.md#822-cli)
  - 匯入時為 `null`
- `頁數`：每個關鍵字抓幾頁
  - 匯入時為 `null`
