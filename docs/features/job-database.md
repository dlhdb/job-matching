...
| ID | job-database |
| 狀態 | ✅ 已完成 |
| 依賴 | 104-job-scraper（沿用其[欄位字典](104-job-scraper.md#55-欄位字典)作為資料契約） |
...
- 身為後續的分析流程（job-scoring、trend-analysis），我需要一份以職缺代碼為主鍵、欄位固定的資料表，以便直接用 pandas 或 SQL 查詢。
...
- 104-job-scraper 爬蟲每次執行後，把結果寫入資料庫
...
- **FR-1**：資料庫預設為專案根目錄的 `data/jobs.db`（以腳本位置推算，不受工作目錄影響）。
  - 檔案或目錄不存在時自動建立並初始化 schema。
  - `data/` 列入 `.gitignore`，資料庫不進版控。
- **FR-2**：以 `職缺代碼` 為主鍵寫入 `jobs` 表。
  - 首次出現時間、最後出現時間，以及更新欄位的時機依 [§5.3](#53-寫入規則)。
- **FR-3**：新資料的 `工作內容` 或 `薪資待遇` 為 `null`，而資料庫中已有非 null 的值時，保留舊值。
- **FR-4**：每次抓取或匯入都在 `scrape_runs` 記一筆紀錄。
  - 同一次執行中出現的每筆職缺記到 `run_jobs`。
- **FR-5**：`fetch_104_jobs.py` 寫出 CSV／JSON 後，預設把結果寫進資料庫。
  - `--db` 指定資料庫路徑。
  - `--no-db` 略過寫入。
- **FR-6**：提供 `src/import_jobs.py`，可一次匯入一或多個爬蟲 JSON。
...
- **輸入與輸出**
  - 輸入：104-job-scraper 產出的職缺清單，可以是爬蟲執行中的記憶體資料，或 `output/104/` 的 JSON。欄位依 [104-job-scraper §5.5](104-job-scraper.md#55-欄位字典)。
  - 輸出：`data/jobs.db`，供 job-scoring、trend-analysis 讀取。
...
| `職缺性質` | INTEGER \| null | `0`／`1`／`2`，意義同 [104-job-scraper §5.6](104-job-scraper.md#56-cli)。匯入時為 `null` |
...
- `t` 取自檔名結尾的 `_<YYYYMMDD_HHMMSS>.json`（104-job-scraper 的命名規則，見 [104-job-scraper §5.4](104-job-scraper.md#54-輸出)）。
...
爬蟲新增的參數（其餘參數見 [104-job-scraper §5.6](104-job-scraper.md#56-cli)）：
