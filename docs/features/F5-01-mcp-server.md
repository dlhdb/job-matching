...
| 狀態 | 待實作 |
| 依賴 | F2-01（抓取）、F2-02（職缺資料庫）、F1-01（評分，含[評分結果入庫](F1-01-job-scoring.md#510-評分結果入庫)） |
| 程式碼 | 尚未實作 |
...
- 不必設計 UI，用對話就能完成 UC-01、UC-02 的操作。
...
- 身為求職者，我想要叫 agent 用幾個關鍵字抓一次職缺，以便不必記爬蟲的參數（UC-01）。
- 身為求職者，我想要叫 agent 對剛抓到的職缺評分，並列出分數最高的幾筆與理由，以便只細看少數職缺（UC-02）。
- 身為求職者，我想要問「最近一週新出現、70 分以上的職缺」，以便不必自己寫 SQL（UC-02）。
- 身為求職者，我想要看到某筆職缺的完整內容與各維度的評分理由，以便決定要不要細看（UC-02）。
...
- 公司資訊與評價的 tool（屬 F3）
- 評分結果入庫與快取的設計（屬 [F1-01 §5.10](F1-01-job-scoring.md#510-評分結果入庫)）；本功能只呼叫它
...
- **輸入與輸出**
  - 讀寫 `data/jobs.db`，資料表契約見 [F2-02 §5.2](F2-02-job-database.md#52-資料表)。評分結果的 `job_scores` 表見 [F1-01 §5.10.1](F1-01-job-scoring.md#5101-資料表)，每筆職缺只有最新一列。
  - 抓取沿用 F2-01，輸出檔與頻率限制見 [F2-01 §5.2](F2-01-104-job-scraper.md#52-104-api-的限制)、[§5.4](F2-01-104-job-scraper.md#54-輸出)。
  - 評分沿用 F1-01，流程見 [F1-01 §5.1](F1-01-job-scoring.md#51-評分流程)、[§5.9](F1-01-job-scoring.md#59-整批評分)；個人資料檔讀取 `profile/`，API key 讀取 `.env`。
...
- 評分由專案內的 scorer 執行，不讓 agent 自己打分：
  ...
  - 才能用上 F1-01 的評分快取（見 [F1-01 §5.10.2](F1-01-job-scoring.md#5102-快取)）。
...
- 回傳：摘要（`成功`、`沿用`、`淘汰`、`失敗`、`未處理`，`沿用` 算在 `成功` 裡），以及每筆的 `職缺代碼`、`職缺名稱`、`總分`、`淘汰`、`評語`、`失敗原因`。各維度理由不放在這裡，要看時用 `get_job_detail`。
- 整批評分可能跑好幾分鐘，超過 client 的 tool 逾時。所以用 `limit` 控制每次的筆數，並在每筆開始時送出 MCP progress 通知（client 有提供 progress token 時）。
- client 建立失敗（缺少 API key、不支援的供應商）時回傳 tool error，同 [F1-01 §5.9.1](F1-01-job-scoring.md#591-流程)。
...
- 評分資料取自 `job_scores`，以 `職缺代碼` LEFT JOIN `jobs`；沒有對應列的職缺視為未評分。
- `include_eliminated` 為 `false` 時，排除 `job_scores.淘汰` 為 1 的職缺。

**`get_job_detail`**：查看單筆職缺。

- 參數：`job_no`（str，必填）。
- 回傳：`jobs` 表的全部欄位，加上 `評分`。`評分` 是 `job_scores.評分結果` 解析後的物件，格式同 [F1-01 §5.7](F1-01-job-scoring.md#57-輸出jobscore) 的 JobScore，尚未評分時為 `null`。
- 找不到職缺時回傳 tool error。
...
- **Then**：
  ...
  - 以同一個 `run_id` 再呼叫一次時，假 client 只對失敗那筆被呼叫，摘要的 `沿用` 為 1
