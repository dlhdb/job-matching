...
| 狀態 | 待規劃 |
| 依賴 | F2-01（抓取）、F2-02（職缺資料庫）、F1-01（評分），以及尚未規劃的「F1-01 評分結果入庫」（見 [TODO.md](../../TODO.md#職缺評分範圍外)） |
| 程式碼 | 尚未實作 |
...
- 公司資訊與評價的 tool（屬 F3）
- 評分結果入庫與快取的資料表設計（屬 F1-01 的後續修改，見 [TODO.md](../../TODO.md#職缺評分範圍外)）；本功能只呼叫它
...
- **FR-6**：`get_job_detail` 回傳單筆職缺的全部欄位，以及最新一次評分的各維度分數與理由。
...
- **輸入與輸出**
  - 讀寫 `data/jobs.db`，資料表契約見 [F2-02 §5.2](F2-02-job-database.md#52-資料表)。評分結果的資料表由「F1-01 評分結果入庫」定義，尚未規劃。
...
- 回傳：摘要（`成功`、`淘汰`、`失敗`、`未處理`），以及每筆的 `職缺代碼`、`職缺名稱`、`總分`、`淘汰`、`評語`、`失敗原因`。各維度理由不放在這裡，要看時用 `get_job_detail`。
...
- 參數：`job_no`（str，必填）。
- 回傳：`jobs` 表的全部欄位，加上 `評分`。`評分` 的格式同 [F1-01 §5.7](F1-01-job-scoring.md#57-輸出jobscore) 的 JobScore，尚未評分時為 `null`。
- 找不到職缺時回傳 tool error。
...
- [ ] 「F1-01 評分結果入庫」尚未規劃。它定案後，才能依其資料表定稿 `score_jobs`、`query_jobs`、`get_job_detail` 的讀寫與回傳欄位。（先規劃 F1-01 的修改，再回頭定稿本文件）
