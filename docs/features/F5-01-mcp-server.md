...
| 狀態 | 待實作 |
| 依賴 | F2-01（抓取）、F2-02（職缺資料庫）、F1-01（評分，含[將職缺評分儲存至資料庫](F1-01-job-scoring.md#8-將職缺評分儲存至資料庫store)） |
| 程式碼 | 尚未實作 |
...
- 公司資訊與評價的 tool（屬 F3）
- 評分結果入庫與快取的設計（屬 [F1-01 §8](F1-01-job-scoring.md#8-將職缺評分儲存至資料庫store)）；本功能只呼叫它
...
- **輸入與輸出**
  - 讀寫 `data/jobs.db`，資料表契約見 [F2-02 §5.2](F2-02-job-database.md#52-資料表)。評分結果的 `job_scores` 表見 [F1-01 §8.2.1](F1-01-job-scoring.md#821-資料表)，每筆職缺只有最新一列。
  - 抓取沿用 F2-01，輸出檔與頻率限制見 [F2-01 §5.2](F2-01-104-job-scraper.md#52-104-api-的限制)、[§5.4](F2-01-104-job-scraper.md#54-輸出)。
  - 評分沿用 F1-01，流程見 [F1-01 §6.2.1](F1-01-job-scoring.md#621-評分流程)、[§7](F1-01-job-scoring.md#7-對整批職缺評分batch)；個人資料檔讀取 `profile/`，API key 讀取 `.env`。
...
- 評分由專案內的 scorer 執行，不讓 agent 自己打分：
  ...
  - 才能用上 F1-01 的評分快取（見 [F1-01 §8.2.2](F1-01-job-scoring.md#822-快取)）。
...
- 回傳：摘要（`成功`、`沿用`、`淘汰`、`失敗`、`未處理`；`沿用` 是沿用上次 AI 評分的筆數，見 [F1-01 §8.2.2](F1-01-job-scoring.md#822-快取)，算在 `成功` 裡），以及每筆的 `職缺代碼`、`職缺名稱`、`總分`、`淘汰`、`評語`、`失敗原因`。各維度理由不放在這裡，要看時用 `get_job_detail`。
- 整批評分可能跑好幾分鐘，超過 client 的 tool 逾時。所以用 `limit` 控制每次的筆數，並在每筆開始時送出 MCP progress 通知（client 有提供 progress token 時）。
- client 建立失敗（缺少 API key、不支援的供應商）時回傳 tool error，同 [F1-01 §7.2.1](F1-01-job-scoring.md#721-流程)。
...
- 參數：`job_no`（str，必填）。
- 回傳：`jobs` 表的全部欄位，加上 `評分`。`評分` 是 `job_scores.評分結果` 解析後的物件，格式同 [F1-01 §9.2.2](F1-01-job-scoring.md#922-jobscore-輸出格式) 的 JobScore，尚未評分時為 `null`。
- 找不到職缺時回傳 tool error。
