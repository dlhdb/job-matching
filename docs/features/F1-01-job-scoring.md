...
〔規劃中〕評分結果目前只寫成檔案，每次執行都會把整批職缺重新交給 AI 評分，重跑同一批就要重付一次費用；其他介面（[F5-01](F5-01-mcp-server.md)）也無法查詢分數。因此評分結果改為同時寫入職缺資料庫，送給 AI 的內容沒有改變的職缺直接沿用上次的 AI 評分。
...
**範圍內**：見 [§4 設計總覽](#4-設計總覽)的系統能力清單，以及 [§9 共用設計](#9-共用設計)。
...
- 輸入的職缺：F2-01 輸出的 JSON（以 `--jobs` 指定），欄位依 [F2-01 §5.5](F2-01-104-job-scraper.md#55-欄位字典)，各維度實際看哪些欄位見 [§6.2.2](#622-評分維度)。
- 輸入的個人資料：`profile/preferences.yaml` 與 `profile/experience.md`，格式見 [§5.2](#52-設計)。由使用者自己維護，不來自其他功能。
- 〔規劃中〕輸入的上次評分：F2-02 資料庫中 `job_scores` 的 AI 維度與評語，用於沿用（見 [§8.2.2](#822-快取)）。
- 單筆輸出：`JobScore` 的 JSON 印到 stdout，欄位見 [§9.2.2](#922-jobscore-輸出格式)。
- 整批輸出：`output/scores/` 下同名的 JSON 與 CSV，欄位見 [§7.2.2](#722-結果檔)。給使用者用 pandas 或 Excel 排序、篩選，本功能不提供瀏覽介面。
- 〔規劃中〕整批與單筆的結果都寫入 F2-02 資料庫的 `job_scores` 表（見 [§8.2.1](#821-資料表)），供 [F5-01](F5-01-mcp-server.md) 查詢。

**系統能力清單**

本功能範圍內的系統能力，細節見各系統能力的「需求」。

| 系統能力 | slug | 狀態 | 章節 | 做什麼 |
| :--- | :--- | :--- | :--- | :--- |
| 載入個人偏好與經歷 | `profile` | ✅ | [§5](#5-載入個人偏好與經歷profile) | 讀取並驗證 `profile/` 的偏好與經歷檔，並提供 `.example` 範本 |
| 對單筆職缺評分 | `score` | ✅ | [§6](#6-對單筆職缺評分score) | 硬性淘汰、四個維度計分（三個由 AI 判斷、薪資由程式計算）與加權總分，以及提示詞模板 |
| 對整批職缺評分 | `batch` | ✅ | [§7](#7-對整批職缺評分batch) | 逐筆套用單筆評分，單筆失敗不中斷，結果寫成 JSON 與 CSV 並印出摘要 |
| 將職缺評分儲存至資料庫 | `store` | 〔規劃中〕 | [§8](#8-將職缺評分儲存至資料庫store) | 結果寫入職缺資料庫，送給 AI 的內容沒變就沿用上次的 AI 評分；CLI 的 `--dry-run` 改為試跑 |

[§9 共用設計](#9-共用設計)不是系統能力，放兩個以上系統能力都會用到的東西：模組佈局、`JobScore` 格式、LLM 供應商抽象層與 Gemini 實作、CLI 的單筆與整批入口、共用測試資料。

## 5. 載入個人偏好與經歷（profile）

### 5.1 需求

- **FR-profile-load**：從指定目錄讀取 `preferences.yaml` 與 `experience.md`。
  - `preferences.yaml` 依 [§5.2.1](#521-preferencesyaml-欄位字典) 驗證，缺少欄位、型別錯誤或權重的鍵不符時報錯，不補預設值。
  - 真實個人資料檔不進版控，版控中提供 `.example` 範本。
...
| `權重` | dict[str, float] | 鍵必須剛好是 [§6.2.2](#622-評分維度) 的四個維度名稱，值 ≥ 0，且總和 > 0。不要求總和為 1 |
...
#### AC-profile-load：個人資料檔驗證與版控（涵蓋 FR-profile-load、FR-cli）
...
- **When**：以 `--dry-run` 對 `ok` 職缺呼叫 CLI 的 `main`（同時指定 `--job-no`）
  - 〔規劃中〕實作 store 後 `--dry-run` 會呼叫 AI，改為已禁止建立 client、不使用 `--dry-run`
...
- **FR-score-filter**：依 [§6.2.4](#624-硬性淘汰規則) 檢查硬性淘汰條件。
  - 收集所有成立的淘汰原因。
  - 被淘汰的職缺不呼叫 AI。
- **FR-score-salary**：依 [§6.2.5](#625-薪資換算與計分) 換算月薪並計算薪資分數。
  - 面議、時薪、日薪、論件計酬與缺值都視為未知（`null`）。
  - 薪資未知不會因此被淘汰。
- **FR-score-ai**：由 AI 依 [§6.2.3](#623-ai-維度的錨點描述) 的錨點，為三個 AI 維度各給 1–5 分或 `null`。
  - 每個維度都附上理由，另外提供總評。
  - AI 回應必須通過 schema 驗證，否則視為失敗。
- **FR-score-total**：依 [§6.2.6](#626-總分) 計算 0–100 的加權總分。
...
3. 依 [§6.2.5](#625-薪資換算與計分) 換算出的月薪上限存在，且小於 `底線月薪` → 原因 `薪資上限 <金額> 低於底線 <金額>`
...
| `年薪` | 下限、上限都除以 `年薪換算月數`，四捨五入到整數（見 [§6.2.6](#626-總分) 的進位規則） | `年薪700,000~1,000,000元` / 700000 / 1000000 |
...
**計分**——設換算後的月薪下限為 `low`，上限為 `high`（可能是「無」）：
...
「`high` < `底線月薪`」的情況已經在 [§6.2.4](#624-硬性淘汰規則) 被淘汰，不會進到計分。
...
4. 分數為 `null` 的維度名稱依 [§6.2.2](#622-評分維度) 的順序列入 `未知維度`，讓使用者知道哪些分數是代入的；`維度` 中的分數仍保留 `null`，`評語` 維持 AI 的原文，不另外加註。

#### 6.2.7 提示詞設計

- 模板放在 `prompts/scoring.md`，和程式碼分開，方便反覆調整。
- **system**：評分者角色、[§6.2.3](#623-ai-維度的錨點描述) 的錨點描述，以及以下規則：
  - 只根據提供的資料判斷
...
LLM 必須輸出以下 JSON（Pydantic 模型 `AIAssessment`）。欄位使用英文名稱，讓 schema 對各家模型都比較穩定，再由 `scorer.py` 轉成 [§9.2.2](#922-jobscore-輸出格式) 的中文鍵名：
...
#### AC-score-salary-filter：薪資計分與硬性淘汰（涵蓋 FR-score-filter、FR-score-salary）

- **Given**：共用的測試偏好（期望 70,000、底線 55,000、年薪 ÷14、排除「乙公司」與「業務」）
- **When**：以下表的職缺呼叫 `score_salary`、`check_hard_filters`（以 `parametrize` 實作）
- **Then**：分數與淘汰結果符合下表與 [§6.2.4](#624-硬性淘汰規則)、[§6.2.5](#625-薪資換算與計分)
...
#### AC-score-flow：評分流程與 AI 回應處理（涵蓋 FR-score-filter、FR-score-ai、FR-score-total）
...
#### AC-score-eliminated：被淘汰的職缺不需要 API key（涵蓋 FR-score-filter、FR-cli）
...
#### AC-score-real：真實評分 〔需網路〕（涵蓋 FR-score-ai、FR-score-total、FR-cli）
...
- **Then**：
  - 輸出符合 [§9.2.2](#922-jobscore-輸出格式)，四個維度依序出現
...
- 〔規劃中〕每筆的「§6.2.1 評分流程」改為 [§8.2.3](#823-流程)：評分前先查快取，評完寫入資料庫。
...
- 結果檔要能直接用來挑職缺，所以每筆除了 [§9.2.2](#922-jobscore-輸出格式) 的 `JobScore` 欄位，還從輸入職缺帶入 `職缺名稱`、`公司名稱`、`薪資待遇`、`職缺連結`，並多一個 `失敗原因` 欄位。

| 欄位 | 型態 | 說明 |
| :--- | :--- | :--- |
| `職缺代碼` | str | |
| `職缺名稱`、`公司名稱`、`薪資待遇`、`職缺連結` | 同 [F2-01 §5.5](F2-01-104-job-scraper.md#55-欄位字典) | 從輸入職缺原樣帶入 |
| `淘汰`、`淘汰原因`、`維度`、`總分`、`未知維度`、`評語` | 同 [§9.2.2](#922-jobscore-輸出格式) | 評分失敗時：`淘汰` 為 false，清單為空，其餘為 `null` |
...
- **CSV**：`utf-8-sig`，每筆一列，欄位順序如下：
  - `職缺代碼`、`職缺名稱`、`公司名稱`、`薪資待遇`
  - `總分`，以及四個維度的分數（欄名就是維度名稱，依 [§6.2.2](#622-評分維度) 的順序；未知時留空）
  - `未知維度`、`淘汰原因`（清單以 `, ` 合併）
  - `評語`、`失敗原因`、`職缺連結`
...
〔規劃中〕有職缺沿用上次的 AI 評分時，第一行之後多印 `[i] 沿用上次的 AI 評分：<筆數> 筆`（見 [§8.2.3](#823-流程)）。

### 7.3 驗收

#### AC-batch-order：整批評分（涵蓋 FR-score-filter、FR-batch-order）
...
#### AC-batch-failure：單筆失敗不中斷整批（涵蓋 FR-batch-failure）
...
#### AC-batch-output：結果檔（涵蓋 FR-batch-output）
...
- **Then**：
  ...
  - JSON 可解析，每個元素的鍵與順序符合 [§7.2.2](#722-結果檔) 的表
  - CSV 以 `utf-8-sig` 讀取時，表頭符合 [§7.2.2](#722-結果檔) 的欄位順序、列的順序與 JSON 相同，未知維度的分數欄是空字串，被淘汰那列的 `淘汰原因` 以 `, ` 合併
- **驗證方式**：`uv run pytest tests/test_job_scoring_batch.py -k output`
- **通過條件**：全部 passed。

#### AC-batch-cli：整批 CLI 與摘要（涵蓋 FR-cli、FR-batch-summary）

- **Given**：共用的測試資料，輸出目錄指到 `tmp_path`
- **When**：
  - (a) 以假 client 取代 `get_client`，不指定 `--job-no` 呼叫 `main`
  - (b) 使用 `--dry-run` 但不指定 `--job-no`。〔規劃中〕實作 store 後刪除此項，整批試跑改由 AC-store-dry-run 驗證
...
- **Then**：
  ...
  - (b) 回傳 1，stderr 含 `[-]`，沒有產生結果檔。〔規劃中〕實作 store 後刪除此項
  - (c) 回傳 0，結果檔中該筆 `淘汰` 為 true
  - (d) 回傳 1，stderr 含 `[-]` 與 `GEMINI_API_KEY`，沒有產生結果檔
- **驗證方式**：`uv run pytest tests/test_score_job_cli.py -k batch`
- **通過條件**：全部 passed。

#### AC-batch-real：真實整批評分 〔需網路〕（涵蓋 FR-batch-order、FR-batch-failure、FR-batch-output、FR-batch-summary）
...
- **FR-store-write**：單筆與整批評分的結果都寫入 `job_scores` 表，格式依 [§8.2.1](#821-資料表)。
  - 每筆職缺只保留最新一次的結果。
  - 評分失敗的職缺不寫入，保留原本的列。
- **FR-store-cache**：沒被淘汰的職缺，評分前依 [§8.2.2](#822-快取) 計算快取鍵。
  - `job_scores` 中同一筆職缺的快取鍵相同時，沿用該列的 AI 維度與評語，不呼叫 AI。
  - 硬性淘汰、薪資分數與加權總分照常重算。
- **FR-store-db-path**：CLI 以 `--db` 指定資料庫路徑，預設為 `data/jobs.db`（同 [F2-02 FR-1](F2-02-job-database.md#4-功能需求)）。
  - 評分結果都會寫入資料庫。
  - 指定 `--dry-run` 時照常完整評分（會呼叫 AI），但不讀寫資料庫，也不沿用上次的 AI 評分。
...
- 職缺內容更新不代表要重新呼叫 AI，是否重問由 [§8.2.2](#822-快取) 的快取鍵決定。
...
| `淘汰` | INTEGER | `0`／`1` |
| `總分` | INTEGER \| null | 同 [§9.2.2](#922-jobscore-輸出格式)，被淘汰時為 `null` |
| `評語` | TEXT \| null | 同 [§9.2.2](#922-jobscore-輸出格式) |
| `評分結果` | TEXT | 整份 `JobScore` 的 JSON（`ensure_ascii=False`），格式同 [§9.2.2](#922-jobscore-輸出格式)；要看各維度的分數與理由時讀這欄 |
| `快取鍵` | TEXT \| null | 見 [§8.2.2](#822-快取)；被淘汰時為 `null` |
...
- `評分結果` 中的 AI 三個維度與 `評語` 是 [§8.2.2](#822-快取) 沿用的來源，其餘欄位每次重算後覆寫。
- 不記錄評分時使用的偏好檔與經歷內容，只記錄供應商與模型（見 [TODO.md](../../TODO.md#職缺評分範圍外)）。

#### 8.2.2 快取

快取的對象只有 **AI 的三個維度與評語**：AI 的輸出完全由送出去的東西決定，所以快取鍵是以下四個字串組成 JSON 陣列後的 SHA-256（十六進位）：
...
- system 提示詞包含模板全文，user 提示詞包含 `experience.md` 全文、`目標方向`、`產業偏好` 與職缺的六個欄位（見 [§6.2.7](#627-提示詞設計)）。所以經歷、提示詞模板、這三項偏好、職缺的這六個欄位與模型任一項改變，快取鍵就會不同，不必另外維護提示詞版本號。
- 程式端的計算（硬性淘汰、薪資分數、加權總分）**每次都重算**，不吃快取，所以以下都不進快取鍵：
  - `薪資待遇`、`薪資下限`、`薪資上限`：不會送進 AI（見 [§6.2.7](#627-提示詞設計)）。職缺改了薪資後，重跑會得到新的薪資分數與總分，但不必重新呼叫 AI。
  - `權重`、`薪資` 門檻、`淘汰條件`：只影響程式端，改了會重算總分，也不必重新呼叫 AI。
- 快取鍵不包含程式碼。修改 [§6.2.4](#624-硬性淘汰規則)、[§6.2.5](#625-薪資換算與計分)、[§6.2.6](#626-總分) 的計算規則會直接生效（因為每次重算），但修改 AI 回應的解析方式後，要清空 `job_scores` 才會重跑。
- 被淘汰的職缺不呼叫 AI，不需要快取，每次都重新判斷並覆寫。
- 快取不設期限。

#### 8.2.3 流程

每筆職缺套用以下流程，取代 [§6.2.1](#621-評分流程) 的直接評分：
...
- 沿用的只有 AI 的三個維度（分數與理由）與評語，其餘照 [§6.2.1](#621-評分流程) 重算，所以薪資或權重改變時分數會跟著更新（見 [§8.2.2](#822-快取)）。
- 沿用也照常覆寫該列，`評分時間` 是這列最後一次寫入的時間；`供應商`、`模型` 來自快取鍵，沿用前後相同。
- 每筆職缺的寫入各自是一個 transaction。整批中途中止時，已經評完的職缺留在資料庫，下次執行會沿用，不必重付費用。
- 整批評分中途發生 SQLite 錯誤時，比照 [§7.2.1](#721-流程) 的其他例外中止，回傳 1、不寫結果檔。已寫入的職缺留在資料庫，重跑時沿用。
- LLM client 仍在第一次需要呼叫 AI 時才建立，所以整批都被淘汰或都沿用快取時，不需要 API key。
- 沿用的職缺照常出現在單筆輸出與整批結果檔中。
- 整批評分函式的每筆回傳多帶一個「沿用」標記，供摘要統計與 [F5-01](F5-01-mcp-server.md) 的 `score_jobs` 使用；結果檔不放這個欄位。
- 單筆評分沿用時，在 stderr 印出 `[i] 沿用上次的 AI 評分結果，未呼叫 AI`。
- 整批摘要在 [§7.2.3](#723-摘要) 的第一行之後多印一行 `[i] 沿用上次的 AI 評分：<筆數> 筆`，這些筆數算在「成功」裡。
- `--dry-run` 不開啟資料庫，也不計算快取鍵，直接走 [§6.2.1](#621-評分流程) 的流程。

### 8.3 驗收

#### AC-store-write：評分結果入庫（涵蓋 FR-store-write）

- **Given**：`tmp_path` 的資料庫中，已有「會評分失敗」那筆職缺的舊列；三筆職缺，依序為會評分成功、會被淘汰、會評分失敗；整批用的假 LLM client
- **When**：呼叫整批評分函式
- **Then**：
  - 成功與淘汰的兩筆寫入 `job_scores`，`評分結果` 解析後符合 [§9.2.2](#922-jobscore-輸出格式)，`淘汰`、`總分`、`評語` 與其中的值一致
  - 淘汰那列的 `總分`、`快取鍵` 為 `null`
  - 評分失敗那筆的舊列內容不變
- **驗證方式**：`uv run pytest tests/test_job_scoring_batch.py -k store`
- **通過條件**：全部 passed。

#### AC-store-cache：沿用上次的 AI 評分（涵蓋 FR-batch-summary、FR-store-cache）
...
#### AC-store-dry-run：dry-run 試跑（涵蓋 FR-cli、FR-store-db-path）
...
#### AC-store-db-path：資料庫路徑（涵蓋 FR-store-db-path）
...
兩個以上系統能力都會用到的需求、設計與驗收。

### 9.1 需求

- **FR-cli**：提供 `src/score_job.py` CLI，參數與結束碼依 [§9.2.4](#924-cli)。
  - 指定 `--job-no` 時評單筆並印出結果，省略時評整批。
  - `--dry-run` 只印出提示詞、不建立 LLM client，必須搭配 `--job-no`。
  - 〔規劃中〕`--dry-run` 依 FR-store-db-path 試跑，單筆與整批都可使用。
  - 〔規劃中〕不再提供只印出提示詞的功能。
...
單筆評分的輸出模型，整批結果檔（[§7.2.2](#722-結果檔)）與 `job_scores`（[§8.2.1](#821-資料表)）都以它為基礎。使用中文鍵名，與爬蟲的輸出一致：

| 欄位 | 型態 | 說明 |
| :--- | :--- | :--- |
| `職缺代碼` | str | 對應到輸入職缺 |
| `淘汰` | bool | 是否被硬性淘汰 |
| `淘汰原因` | list[str] | [§6.2.4](#624-硬性淘汰規則) 的原因；沒被淘汰時是空清單 |
| `維度` | dict[str, {分數: int \| null, 理由: str}] \| null | 鍵依 [§6.2.2](#622-評分維度) 的順序排列；被淘汰時為 `null` |
...
- API key 從環境變數 `GEMINI_API_KEY` 讀取，由 `GeminiClient` 建構時檢查，**沒有設定或是空字串**就拋出例外。CLI 只在「沒被淘汰、也不是 `--dry-run`」時才建立 client，所以只有真的要呼叫 AI 時才需要 key。〔規劃中〕沿用上次 AI 評分的職缺也不建立 client（見 [§8.2.3](#823-流程)）。
...
| `--job-no` | 指定時只評這筆職缺，找不到時印出 `[-]` 並結束；省略時評整批（見 [§7](#7-對整批職缺評分batch)） |
...
| `--dry-run` | 先執行硬性淘汰與薪資計分，再印出完整的 system 與 user 提示詞，**不建立 LLM client、不發出網路請求**。必須搭配 `--job-no`，否則印出 `[-]` 並結束。〔規劃中〕改為試跑：完整評分（會呼叫 AI），但不讀寫資料庫、不沿用上次的 AI 評分，不影響已存的正式分數。單筆照常印到 stdout，整批照常寫出結果檔。用於換一份偏好檔、經歷或模型，實際看 AI 會給幾分、理由是什麼 |
| `--db` | 〔規劃中〕評分結果寫入的資料庫，預設為專案根目錄的 `data/jobs.db`（見 [§8](#8-將職缺評分儲存至資料庫store)） |
...
- `1`：以下情況會印出 `[-]` 並結束
  - 個人資料檔缺少或格式錯誤
  - 找不到職缺
  - 使用 `--dry-run` 但沒有指定 `--job-no`（〔規劃中〕實作 store 後移除）
  - 不支援的供應商
  - 缺少 API key
  - 單筆評分時 Gemini API 呼叫失敗、沒有回傳文字，或 AI 回應格式錯誤
  - 〔規劃中〕無法開啟資料庫，或寫入資料庫時發生 SQLite 錯誤（整批評分的行為見 [§8.2.3](#823-流程)）
...
#### AC-cli-dry-run：dry-run 印出提示詞且不建立 client（涵蓋 FR-cli、FR-llm）

〔規劃中〕實作 store 後由 AC-store-dry-run 取代。
...
#### AC-cli-error：錯誤處理與供應商隔離（涵蓋 FR-cli、FR-llm）
