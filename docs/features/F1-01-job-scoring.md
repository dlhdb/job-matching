...
- 輸入的職缺：F2-01 輸出的 JSON（以 `--jobs` 指定），欄位依 [F2-01 §5.5](F2-01-104-job-scraper.md#55-欄位字典)，各維度實際看哪些欄位見 [§4.2.4](#424-評分維度)。
- 輸入的個人資料：`profile/preferences.yaml` 與 `profile/experience.md`，格式見 [§4.2.1](#421-個人資料檔)。由使用者自己維護，不來自其他功能。
- 〔規劃中〕輸入的上次評分：F2-02 資料庫中 `job_scores` 的 AI 維度與評語，用於沿用（見 [§8.2.1](#821-快取鍵)）。
...
- 〔規劃中〕整批與單筆的結果都寫入 F2-02 資料庫的 `job_scores` 表（見 [§7.2.1](#721-資料表)），供 [F5-01](F5-01-mcp-server.md) 查詢。
...
| 使用者故事 | slug | 狀態 | 章節 |
| :--- | :--- | :--- | :--- |
| 評單筆職缺並看懂每個分數 | `score` | ✅ | [§4](#4-評單筆職缺並看懂每個分數score) |
| 明顯不合的職缺直接淘汰 | `filter` | ✅ | [§5](#5-明顯不合的職缺直接淘汰filter) |
| 一次評完整批並拿到結果檔 | `batch` | ✅ | [§6](#6-一次評完整批並拿到結果檔batch) |
| 依分數查詢職缺 | `store` | 〔規劃中〕 | [§7](#7-依分數查詢職缺store) |
| 重跑時不重複付 AI 費用 | `cache` | 〔規劃中〕 | [§8](#8-重跑時不重複付-ai-費用cache) |
| 換設定試跑而不影響正式分數 | `dry-run` | 〔規劃中〕 | [§9](#9-換設定試跑而不影響正式分數dry-run) |
...
| `年薪` | 下限、上限都除以 `年薪換算月數`，四捨五入到整數（見 [§4.2.7](#427-總分) 的進位規則） | `年薪700,000~1,000,000元` / 700000 / 1000000 |
...
- **Then**：薪資分數符合下表與 [§4.2.6](#426-薪資換算與計分)，而且都不會被淘汰
  ...
  | `年薪980,000~1,260,000元`（換算 70,000~90,000） | 980000 | 1260000 | 5 |
  | `年薪980,007~1,260,007元`（換算 70,000.5~90,000.5，理由中為 70,001–90,001） | 980007 | 1260007 | 5 |
  | `月薪50,000元以上`（沒有上限） | 50000 | 9999999 | 2 |
  | `待遇面議` | 0 | 0 | `None` |
...
- **When**：以下表的職缺呼叫 `check_hard_filters`
- **Then**：淘汰原因符合下表與 [§5.2.1](#521-硬性淘汰規則)；職稱關鍵字不分大小寫（關鍵字 `sales` 會淘汰 `Senior SALES Manager`）
...
- **FR-store-write**：單筆與整批評分的結果都寫入 `job_scores` 表，格式依 [§7.2.1](#721-資料表)。
  - 每筆職缺只保留最新一次的結果。
  - 評分失敗的職缺不寫入，保留原本的列。
- **FR-store-db-path**：CLI 以 `--db` 指定資料庫路徑，預設為 `data/jobs.db`（與 [F2-02 §4](F2-02-job-database.md#4-功能需求) 的預設路徑相同）。
