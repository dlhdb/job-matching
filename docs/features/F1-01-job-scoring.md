...
- 〔規劃中〕評分結果寫入職缺資料庫，送給 AI 的內容沒有改變時沿用上次的 AI 評分
...
- **FR-1**：從指定目錄讀取 `preferences.yaml` 與 `experience.md`。
  - `preferences.yaml` 依 [§5.2.1](#521-preferencesyaml-欄位字典) 驗證，缺少欄位、型別錯誤或權重的鍵不符時報錯，不補預設值。
  - 真實個人資料檔不進版控，版控中提供 `.example` 範本。
- **FR-2**：依 [§5.4](#54-硬性淘汰規則) 檢查硬性淘汰條件。
  - 收集所有成立的淘汰原因。
  - 被淘汰的職缺不呼叫 AI。
- **FR-3**：依 [§5.5](#55-薪資換算與計分) 換算月薪並計算薪資分數。
  - 面議、時薪、日薪、論件計酬與缺值都視為未知（`null`）。
  - 薪資未知不會因此被淘汰。
- **FR-4**：由 AI 依 [§5.3.1](#531-ai-維度的錨點描述) 的錨點，為三個 AI 維度各給 1–5 分或 `null`。
  - 每個維度都附上理由，另外提供總評。
  - AI 回應必須通過 schema 驗證，否則視為失敗。
- **FR-5**：依 [§5.6](#56-總分) 計算 0–100 的加權總分。
  - 分數為 `null` 的維度以 3 分代入，讓所有沒被淘汰的職缺都有可以互相比較的總分。
- **FR-6**：提供 `src/score_job.py` CLI，參數與結束碼依 [§5.8.5](#585-cli)。
  - 指定 `--job-no` 時評單筆並印出結果，省略時評整批。
  - 〔規劃中〕`--dry-run` 依 FR-14 試跑，單筆與整批都可使用。
  - 〔規劃中〕不再提供只印出提示詞的功能。
- **FR-7**：LLM 呼叫透過 `LLMClient` 介面與 `get_client(provider, model)` 工廠函式，預設使用 Gemini。
...
- **FR-10**：整批結果寫成 JSON 與 CSV 兩個檔案，位置與格式依 [§5.9.2](#592-結果檔)。
  - CSV 使用 `utf-8-sig`，讓 Excel 能正常顯示中文。
- **FR-11**：整批評分結束時印出摘要：總筆數、成功、淘汰、失敗筆數，以及兩個結果檔的路徑。
  - 〔規劃中〕成功筆數中沿用上次 AI 評分的筆數另外列出。
- **FR-12**〔規劃中〕：單筆與整批評分的結果都寫入 `job_scores` 表，格式依 [§5.10](#510-評分結果入庫)。
  - 每筆職缺只保留最新一次的結果。
  - 評分失敗的職缺不寫入，保留原本的列。
- **FR-13**〔規劃中〕：沒被淘汰的職缺，評分前依 [§5.10.2](#5102-快取) 計算快取鍵。
  - `job_scores` 中同一筆職缺的快取鍵相同時，沿用該列的 AI 維度與評語，不呼叫 AI。
  - 硬性淘汰、薪資分數與加權總分照常重算。
- **FR-14**〔規劃中〕：CLI 以 `--db` 指定資料庫路徑，預設為 `data/jobs.db`（同 [F2-02 FR-1](F2-02-job-database.md#4-功能需求)）。
  - 評分結果都會寫入資料庫。
...
- **輸入與輸出**
  - 輸入的職缺：F2-01 輸出的 JSON（以 `--jobs` 指定），欄位依 [F2-01 §5.5](F2-01-104-job-scraper.md#55-欄位字典)，各維度實際看哪些欄位見 [§5.3](#53-評分維度)。
  - 輸入的個人資料：`profile/preferences.yaml` 與 `profile/experience.md`，格式見 [§5.2](#52-輸入個人資料檔)。由使用者自己維護，不來自其他功能。
  - 〔規劃中〕輸入的上次評分：F2-02 資料庫中 `job_scores` 的 AI 維度與評語，用於沿用（見 [§5.10.2](#5102-快取)）。
  - 單筆輸出：`JobScore` 的 JSON 印到 stdout，欄位見 [§5.7](#57-輸出jobscore)。
  - 整批輸出：`output/scores/` 下同名的 JSON 與 CSV，欄位見 [§5.9.2](#592-結果檔)。給使用者用 pandas 或 Excel 排序、篩選，本功能不提供瀏覽介面。
  - 〔規劃中〕整批與單筆的結果都寫入 F2-02 資料庫的 `job_scores` 表（見 [§5.10.1](#5101-資料表)），供 [F5-01](F5-01-mcp-server.md) 查詢。
...
- API key 從環境變數 `GEMINI_API_KEY` 讀取，由 `GeminiClient` 建構時檢查，**沒有設定或是空字串**就拋出例外。CLI 只在「沒被淘汰」時才建立 client，所以只有真的要呼叫 AI 時才需要 key。〔規劃中〕沿用上次 AI 評分的職缺也不建立 client（見 [§5.10.3](#5103-流程)）。
...
| `--dry-run` | 〔規劃中〕試跑：完整評分（會呼叫 AI），但不讀寫資料庫、不沿用上次的 AI 評分，不影響已存的正式分數。單筆照常印到 stdout，整批照常寫出結果檔。用於換一份偏好檔、經歷或模型，實際看 AI 會給幾分、理由是什麼 |
...
- `1`：以下情況會印出 `[-]` 並結束
  ...
  - 〔規劃中〕無法開啟資料庫，或寫入資料庫時發生 SQLite 錯誤（整批評分的行為見 [§5.10.3](#5103-流程)）
...
- 〔規劃中〕每筆的「§5.1 評分流程」改為 [§5.10.3](#5103-流程)：評分前先查快取，評完寫入資料庫。
...
〔規劃中〕有職缺沿用上次的 AI 評分時，第一行之後多印 `[i] 沿用上次的 AI 評分：<筆數> 筆`（見 [§5.10.3](#5103-流程)）。
...
- 職缺內容更新不代表要重新呼叫 AI，是否重問由 [§5.10.2](#5102-快取) 的快取鍵決定。
...
- `評分結果` 中的 AI 三個維度與 `評語` 是 [§5.10.2](#5102-快取) 沿用的來源，其餘欄位每次重算後覆寫。
...
- system 提示詞包含模板全文，user 提示詞包含 `experience.md` 全文、`目標方向`、`產業偏好` 與職缺的六個欄位（見 [§5.8.2](#582-提示詞設計)）。所以經歷、提示詞模板、這三項偏好、職缺的這六個欄位與模型任一項改變，快取鍵就會不同，不必另外維護提示詞版本號。
- 程式端的計算（硬性淘汰、薪資分數、加權總分）**每次都重算**，不吃快取，所以以下都不進快取鍵：
  - `薪資待遇`、`薪資下限`、`薪資上限`：不會送進 AI（見 [§5.8.2](#582-提示詞設計)）。職缺改了薪資後，重跑會得到新的薪資分數與總分，但不必重新呼叫 AI。
  - `權重`、`薪資` 門檻、`淘汰條件`：只影響程式端，改了會重算總分，也不必重新呼叫 AI。
- 快取鍵不包含程式碼。修改 §5.4、§5.5、§5.6 的計算規則會直接生效（因為每次重算），但修改 AI 回應的解析方式後，要清空 `job_scores` 才會重跑。
...
- 沿用的只有 AI 的三個維度（分數與理由）與評語，其餘照 [§5.1](#51-評分流程) 重算，所以薪資或權重改變時分數會跟著更新（見 [§5.10.2](#5102-快取)）。
- 沿用也照常覆寫該列，`評分時間` 是這列最後一次寫入的時間；`供應商`、`模型` 來自快取鍵，沿用前後相同。
- 每筆職缺的寫入各自是一個 transaction。整批中途中止時，已經評完的職缺留在資料庫，下次執行會沿用，不必重付費用。
- 整批評分中途發生 SQLite 錯誤時，比照 [§5.9.1](#591-流程) 的其他例外中止，回傳 1、不寫結果檔。已寫入的職缺留在資料庫，重跑時沿用。
...
- 整批摘要在 [§5.9.3](#593-摘要) 的第一行之後多印一行 `[i] 沿用上次的 AI 評分：<筆數> 筆`，這些筆數算在「成功」裡。
...
### AC-15：沿用上次的 AI 評分 〔規劃中〕（涵蓋 FR-11、FR-13）
