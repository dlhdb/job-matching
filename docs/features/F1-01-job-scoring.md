...
- **FR-11**：整批評分結束時印出摘要：總筆數、成功、淘汰、失敗筆數，以及兩個結果檔的路徑。〔規劃中〕成功筆數中沿用上次 AI 評分的筆數另外列出。
- **FR-12**〔規劃中〕：單筆與整批評分的結果都寫入 `job_scores` 表，格式依 [§5.10](#510-評分結果入庫)。每筆職缺只保留最新一次的結果；評分失敗的職缺不寫入，保留原本的列。
- **FR-13**〔規劃中〕：沒被淘汰的職缺，評分前依 [§5.10.2](#5102-快取) 計算快取鍵；`job_scores` 中同一筆職缺的快取鍵相同時，沿用該列的 AI 維度與評語，不呼叫 AI；硬性淘汰、薪資分數與加權總分照常重算。
- **FR-14**〔規劃中〕：CLI 以 `--db` 指定資料庫路徑，預設為 `data/jobs.db`（同 [F2-02 FR-1](F2-02-job-database.md#4-功能需求)）。評分結果都會寫入資料庫；指定 `--dry-run` 時照常完整評分（會呼叫 AI），但不讀寫資料庫，也不沿用上次的 AI 評分。
