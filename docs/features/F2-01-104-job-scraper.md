...
本功能依關鍵字與地區批量抓取 104 職缺，產出結構化資料，作為後續 AI 評分（F1、F4）的輸入。
...
- 其他求職平台（另開 F2-xx）
- 公司評價資訊（屬 F3）
- 排程定期執行、資料庫儲存（屬 F2-02）
- 跨次執行的去重（去重只在單次執行內，見 [TODO.md](../../TODO.md#爬蟲)）
...
- 檔名是 `jobs_104_<關鍵字>_<YYYYMMDD_HHMMSS>.csv` 與 `.json`，寫在 `OUTPUT_DIR`（專案根目錄的 `output/104/`，以腳本位置推算，不受工作目錄影響，不存在時自動建立；已列入 `.gitignore`）。多個關鍵字以 `_` 連接，超過 30 字元就截斷並加上 `_etc`，非英數字元（`-`、`_` 除外）會被移除，結果為空時用 `all`。
...
輸出使用中文鍵名，順序等於 `CSV_FIELDNAMES`。F1-01、F4-01 都以這份字典作為輸入契約；新增欄位時要同步修改 `parse_jobs()`、`CSV_FIELDNAMES` 與本表。
