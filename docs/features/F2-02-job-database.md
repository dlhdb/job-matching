...
- 排程定期執行（屬 F2-03）
- 評分結果存入資料庫、評分結果快取（屬 F1-01 的後續修改，見 [TODO.md](../../TODO.md#職缺評分範圍外)）
- F1-01 改從資料庫讀取職缺（見 [TODO.md](../../TODO.md#職缺評分範圍外)）
- 判斷職缺是否已下架：只記錄最後一次被抓到的時間，不推論是否下架（見 [§5.2](#52-資料表)）
- 其他求職平台的資料（另開 F2-xx）
- 查詢或瀏覽介面：直接用 pandas 或 SQLite 工具讀取
...
**`run_jobs`**：一次執行與其中出現的職缺，主鍵是（`執行編號`, `職缺代碼`）。
...
- **Then**：
  ...
  - 第二次：印出 `[i]` 並略過所有檔案，資料庫內容與第一次相同；結束碼為 0
