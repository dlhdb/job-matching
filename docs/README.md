...
以下指標對應第一個目標，都靠使用者自己手動觀察。第二個目標的指標等 trend-analysis 規劃時再定。
...
flowchart LR
    ...
    company["company-info 公司資訊與評價 (UP-03)"] -.->|公司評價| scoring
    ...
    class company,mcp,trend planned
```

## 功能清單

功能是系統中一個獨立且可交付的大型功能模組，一個功能一份功能文件。功能 ID 用英文簡短描述（kebab-case），也是檔名：`features/<功能 ID>.md`。功能不再分組，理由見 [決策紀錄 0003](decisions/0003-flat-feature-list.md)。

狀態值：`待規劃` → `待實作` → `實作中` → `✅ 已完成`。還沒有功能文件的功能，狀態為 `待規劃`，文件欄寫 `—`。

| ID | 功能 | 定義 | 解決的使用者問題 | 狀態 | 文件 |
| :-- | :--- | :--- | :--- | :--- | :--- |
| 104-job-scraper | 104 職缺爬蟲 | 從 104 擷取職缺，轉換成結構化資料，供 AI 分析。 | UP-01 | ✅ 已完成 | [104-job-scraper.md](features/104-job-scraper.md) |
| job-database | 職缺資料庫 | 把每次抓到的職缺累積到同一個資料庫，跨次去重並記錄出現時間。 | UP-01 | ✅ 已完成 | [job-database.md](features/job-database.md) |
| job-scoring | 職缺評分 | 依評分方法與提示詞，自動給職缺打分並附上簡短評語，讓使用者依總分快速判斷哪些值得細看。 | UP-02 | 待規劃 | [job-scoring.md](features/job-scoring.md) |
| company-info | 擷取公司資訊與評價 | 從網路上擷取公司相關資訊與評價，轉換成結構化資料，供 AI 分析。 | UP-03 | 待規劃 | — |
| trend-analysis | 趨勢與興趣分佈分析 | 從全台、全球角度分析職缺趨勢，並發現自己對職缺或產業的興趣分佈。 | UP-04、UP-05 | 待規劃 | — |
| mcp-server | MCP 介面 | 讓 agent 以 tool 的形式操作抓取、評分與查詢。 | UP-01、UP-02 | 待規劃 | [mcp-server.md](features/mcp-server.md) |
...
- **job-database**：每次抓取的結果會自動累積到 `data/jobs.db`，跨次去重並記錄每筆職缺第一次與最後一次被抓到的時間，以及每次抓取的條件；過去抓好的 JSON 也能匯入。
- **job-scoring**：可對單筆或整批職缺評分，依職涯方向、技能、產業公司、薪資四個維度給出 0–100 總分與評語；薪資低於底線、公司或職稱在排除清單中的職缺直接淘汰。整批評分時單筆失敗不中斷，結果寫成 JSON 與 CSV，可用表格工具排序、篩選。
- **company-info**：尚無。
- **trend-analysis**：尚無。
- **mcp-server**：尚無。
...
5. **驗收**：逐條執行該使用者故事「驗收」小節的驗證方式，回報每條結果（✅ / ❌ 與實際輸出）。全部通過後，移除該使用者故事與共用章節中相關的〔規劃中〕標記，狀態改為 `✅ 已完成`，並更新上方該功能的「現況」。
...
| 快取鍵 | 由送給 AI 的內容算出的雜湊值，相同時沿用上次的 AI 評分 | [job-scoring §8.2.1](features/job-scoring.md#821-快取鍵) |
