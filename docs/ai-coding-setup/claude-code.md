...
- 這份文件說明 Claude Code 專屬的權限設定與理由。
- 容器層級的隔離與防火牆是所有 AI coding 工具共用的設定，見 [devcontainer.md](devcontainer.md)。

## 權限規則（`.claude/settings.json`）

- 跨越任何權限模式（包含未來若切換到 bypass 模式）都要守住的規則，寫在專案共用的 `.claude/settings.json`。
  - 這個檔案會進版控，套用到所有人。
- 比對順序是 `deny` → `ask` → `allow`，`deny` 優先。
...
- `deny`：這個專案裡沒有合理使用情境、且難以復原的操作。
- `ask`：會影響共享狀態（遠端、PR），但有合理使用情境的操作。
- 不設規則：單純危險但本地、可復原的操作（例如一般的 `git commit --amend`），交給日常的權限提示處理。
