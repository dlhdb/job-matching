...
- job-score-database：
  - 每筆職缺的評分紀錄（是否淘汰、總分、評語）保存在職缺資料庫，手動評分每筆職缺只留最新一次。
...
- job-auto-scoring：
  ...
  - 薪資低於底線、公司或職稱在排除清單中的職缺直接淘汰，評語寫出淘汰原因。
  - 每次自動評分有變化就新增一筆紀錄，保留評分歷史，並標示評分來源（自動或手動）；自動與手動評分並存，互不覆寫，也不提供刪除評分紀錄。
  - 同一筆職缺有多筆評分時，依分數列出與取出單筆都以代表的一筆為準（有手動評分用手動，否則用最新的自動評分），並標示評分來源；也能依職缺代碼取出所有評分紀錄，比較 AI 前後的判斷。
  - 整批評分時單筆失敗不中斷，結果寫成 JSON 與 CSV，可用表格工具排序、篩選。
  - 單筆與整批的評分結果都寫進評分資料庫，評分明細包含各維度的分數與理由，可以依職缺代碼取出。
  - 依分數列出時多帶產生評分的供應商與模型。
