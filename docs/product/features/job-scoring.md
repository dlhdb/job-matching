...
- ID：job-scoring
- 狀態：✅ 已完成
- 依賴：104-job-scraper（使用其輸出的職缺 JSON 作為輸入）、job-database（評分結果要能和其中的職缺資料一起查詢）
...
範圍內：見 [§3.2 使用者故事清單](#32-使用者故事清單)，以及 [§10 非功能需求](#10-非功能需求)、[§11 共用規則](#11-共用規則)。
...
[§10 非功能需求](#10-非功能需求)與 [§11 共用規則](#11-共用規則)不是使用者故事。§11 放跨越各章的規則：
...
- 「一筆職缺」是輸入 JSON 陣列中的一個物件，欄位見 [104-job-scraper 的欄位字典](104-job-scraper.md#821-欄位字典)。
