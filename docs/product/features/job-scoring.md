...
- 整批與單筆的結果都會保存下來，可以和職缺資料一起依分數查詢（見 [§7.2.1](#721-儲存的評分資訊)），[mcp-server](mcp-server.md) 也查詢這份資料。
...
調整提示詞的開發者也用單筆評分（`--job-no`，見 [§11.2.2](#1122-cli)）反覆修改。
...
- When：
  ...
  - (c) 沒有設定 API key，職缺檔只有會被淘汰的那筆，不指定 `--job-no`
  - (d) 沒有設定 API key，不指定 `--job-no`
- Then：
  - (a) 正常結束，沒有印出評分結果的 JSON，摘要含 `共 2 筆，成功 1、淘汰 1、失敗 0` 與兩個結果檔的路徑
  - (b) 顯示錯誤訊息並結束，沒有產生結果檔。〔規劃中〕實作 dry-run 試跑後刪除此項
  - (c) 正常結束，結果檔中該筆 `淘汰` 為 true
  - (d) 顯示提到 `GEMINI_API_KEY` 的錯誤訊息並結束，沒有產生結果檔
- 通過條件：全部符合。
...
- 資料庫只放正式評分：〔規劃中〕試跑（換偏好檔、經歷或模型來比較）時要加 `--dry-run`，否則會覆寫正式的分數（見 [§9](#9-換設定試跑而不影響正式分數dry-run)）。
...
- FR-dry-run：指定 `--dry-run` 時照常完整評分（會呼叫 AI），但不讀寫資料庫，也不沿用上次的 AI 評分。
  - 單筆與整批都可使用：
    - 單筆照常印出評分結果。
    - 整批照常寫出結果檔。
  - 取代現行「只印出提示詞、不呼叫 AI」的 `--dry-run`（見 [§11.1](#111-需求)），不再提供只印出提示詞的功能。
...
- API key 從環境變數 `GEMINI_API_KEY` 讀取：
  ...
  - 只有真的要呼叫 AI 時才需要（被淘汰、沿用上次的 AI 評分、`--dry-run` 不需要）。

### 11.3 驗收

#### AC-cli-dry-run：dry-run 印出提示詞且不呼叫 AI

〔規劃中〕實作 dry-run 試跑後由 AC-dry-run 取代。

- Given：沒有設定 API key
- When：以 `--dry-run` 評一筆正常的職缺
- Then：
  - 正常結束
  - 印出的提示詞包含偏好的目標方向、經歷、職缺的工作內容，以及空欄位的「（無資料）」
  - 沒有呼叫 AI
- 通過條件：全部符合。
