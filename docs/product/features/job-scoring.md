...
- ID：job-scoring
- 狀態：✅ 已完成
- 依賴：104-job-scraper（使用其輸出的職缺 JSON 作為輸入）、job-database（依其職缺欄位契約讀取職缺欄位，評分結果也要能和其中的職缺資料一起查詢）
...
- [§10 非功能需求](#10-非功能需求)：只在必要時呼叫 AI、個人資料與 API key 不進版控、資訊不足時分數為 `null`
- [§11 共用規則](#11-共用規則)：評分結果格式、CLI、AI 供應商與 API key
...
- 強制重新評分的參數：見 [TODO.md](../../../TODO.md#職缺評分範圍外)
- 平行呼叫 AI：整批維持循序執行
...
- 單筆輸出：評分結果的 JSON 印在終端機，欄位見 [§11.2.1](#1121-評分結果格式)。
...
- 整批與單筆的結果都會保存下來，可以和職缺資料一起依分數查詢（見 [§7.2.1](#721-儲存的評分資訊)）。試跑的結果不保存。

## 4. 評單筆職缺並看懂每個分數（score）

身為求職者，我想要依照寫成檔案的偏好與經歷替職缺評分，並看到每個維度的分數與理由，以便每次都用同一套標準，並知道總分是怎麼來的、判斷要不要相信它。

調整提示詞的開發者也用單筆評分反覆修改，並加上 `--dry-run`（`--job-no ... --dry-run`，見 [§11.2.2](#1122-cli)）避免覆寫正式的分數。
...
- `profile/experience.md`：工作經歷與技能，原文放進提示詞。
  - 格式是自由格式的 Markdown。
- 兩個檔案都不進版控，版控中只保留 `.example` 範本（見 [§10.1](#101-需求)）。
...
- Then：
  - 輸出符合 [§11.2.1](#1121-評分結果格式)，四個維度依序出現
...
- 每筆的「§4.2.3 評分流程」改為 [§7.2.3](#723-流程)：評完寫入資料庫。
...
- 結果檔要能直接用來挑職缺，所以每筆除了 [§11.2.1](#1121-評分結果格式) 的欄位，還有：
...
- `淘汰`、`淘汰原因`、`維度`、`總分`、`未知維度`、`評語`（同 [§11.2.1](#1121-評分結果格式)）
...
- FR-store-write：單筆與整批評分的結果都寫入資料庫，內容依 [§7.2.1](#721-儲存的評分資訊)。
  - 每筆職缺只保留最新一次的結果。
  - 評分失敗的職缺不寫入，保留原本的結果。
...
- FR-store-list：列出評過分的職缺，結果依 [§7.2.2](#722-列出評過分的職缺的結果)。
  - 排序是總分由高到低。
  - 可以只列出被淘汰的職缺，或只列出沒被淘汰的職缺。
  - 可以指定最多取幾筆、從第幾筆開始。
- FR-store-get：以職缺代碼取出單筆的完整評分結果，格式依 [§11.2.1](#1121-評分結果格式)。
  - 沒有評過這筆職缺時明確表示查不到。
...
- 評分結果保存在職缺資料庫（位置見 [§11.2.2](#1122-cli) 的 `--db`），可以和職缺資料一起以 `職缺代碼` 查詢。
- 每筆職缺一筆紀錄，只保留最新一次成功的評分：
  - 評分時間
  - 是否淘汰、總分、評語：可以依這些值篩選與排序
  - 完整的評分結果（[§11.2.1](#1121-評分結果格式)）：要看各維度的分數與理由時讀這裡
  - 產生 AI 維度時使用的供應商與模型：被淘汰的職缺沒有呼叫 AI，這兩項為 `null`
- 爬蟲重新抓到同一筆職缺時，評分不會被清掉。
- 要整批重評時，清掉評分即可，不影響職缺資料。
- 評分時用的職缺不必先匯入資料庫，照樣寫入。
- 不保留歷史，重新評分時覆寫（見 [TODO.md](../../../TODO.md#職缺評分範圍外)）。
...
- 每筆包含職缺的欄位（依 [job-database §7.2.1](job-database.md#721-職缺欄位契約)）與這筆的評分資訊：評分時間、是否淘汰、總分、評語、供應商與模型。
  - 完整的評分結果不放進列表，要看各維度時以職缺代碼另外取出。
- 排序是總分由高到低。
  - 被淘汰的職缺沒有總分，排在最後。
  - 總分相同時，再以職缺代碼排序，同樣的資料每次查出來的順序才一致。
- 評分時不必先把職缺匯入職缺資料庫（見 [§7.2.1](#721-儲存的評分資訊)），所以可能查到只有評分、沒有職缺資料的職缺，這時職缺的欄位是 `null`。
- 沒有符合條件的職缺時是空清單，不是錯誤。
...
- Then：
  - 成功與淘汰的兩筆寫入資料庫
  - 儲存的完整評分結果符合 [§11.2.1](#1121-評分結果格式)，是否淘汰、總分、評語與其中的值一致
  - 淘汰那筆的總分為 `null`
  - 評分失敗那筆的舊結果不變
- 通過條件：全部符合。
...
#### AC-store-list：列出評過分的職缺

- Given：
  - 三筆評分結果：兩筆沒被淘汰、總分不同，一筆被淘汰
  - 其中一筆只有評分結果，職缺資料不在職缺資料庫裡
- When：
  - 不指定條件列出
  - 只列出被淘汰的職缺
  - 只列出沒被淘汰的職缺
  - 指定最多取 1 筆
- Then：
  - 不指定條件時三筆都在，欄位依 [§7.2.2](#722-列出評過分的職缺的結果)
  - 排序是總分由高到低，被淘汰的那筆排在最後
  - 只有評分結果、沒有職缺資料的那筆，職缺欄位是 `null`，評分資訊照常
  - 指定淘汰與否時，只有符合的職缺
  - 指定筆數時，回傳的筆數不超過指定值
- 通過條件：全部符合。
...
- Then：
  - 評過時得到完整的評分結果，格式符合 [§11.2.1](#1121-評分結果格式)，各維度的分數與理由都在
  - 沒評過時明確表示查不到
- 通過條件：全部符合。
...
查上次的評分與沿用在 [§7.2.3](#723-流程) 的流程中進行。
...
- NFR-privacy：真實的個人資料檔與 API key 不進版控。版控中只提供範本：
  ...
  - `.env` 的範本是 `.env.example`（見 [§11.2.3](#1123-ai-供應商與-api-key)）。
...
- FR-cli：提供 `src/score_job.py` CLI，參數與錯誤訊息依 [§11.2.2](#1122-cli)。
  - 指定 `--job-no` 時評單筆並印出結果，省略時評整批。
  - `--dry-run` 試跑，見 [§9](#9-換設定試跑而不影響正式分數dry-run)。
- FR-llm：AI 供應商以 `--provider` 選擇，規則依 [§11.2.3](#1123-ai-供應商與-api-key)。
  - 預設使用 Gemini。
...
- 整批結果檔（[§6.2.2](#622-結果檔)）與資料庫中的評分結果（[§7.2.1](#721-儲存的評分資訊)）都以它為基礎。
- 使用中文鍵名，與爬蟲的輸出一致。
...
- 無法開啟或寫入資料庫（整批評分的行為見 [§7.2.3](#723-流程)）

整批評分時，個別職缺評分失敗不算錯誤，失敗筆數見摘要。

#### 11.2.3 AI 供應商與 API key
