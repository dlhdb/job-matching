...
- 輸入的職缺：104-job-scraper 輸出的 JSON，以 `--jobs` 指定。
  - 欄位依 [job-database §6.2.1](job-database.md#621-職缺欄位契約)。
...
- 整批與單筆的結果都會保存下來，可以和職缺資料一起依分數查詢（見 [§7.2.1](#721-儲存的評分資訊)），[mcp-server](mcp-server.md) 也查詢這份資料。試跑的結果不保存。
...
- 「一筆職缺」是輸入 JSON 陣列中的一個物件，欄位見 [job-database 的職缺欄位契約](job-database.md#621-職缺欄位契約)。
...
- `職缺代碼`（str）
- `職缺名稱`、`公司名稱`、`薪資待遇`、`職缺連結`（同 [job-database §6.2.1](job-database.md#621-職缺欄位契約)）：從輸入職缺原樣帶入
...
- FR-store-db-path：CLI 以 `--db` 指定資料庫路徑。
  - 預設為 `data/jobs.db`，與 [job-database §6.1](job-database.md#61-需求) 的預設路徑相同。
