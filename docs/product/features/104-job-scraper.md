...
- ID：104-job-scraper
- 狀態：✅ 已完成
- 依賴：job-database（依其[職缺欄位契約](job-database.md#621-職缺欄位契約)整理欄位，並依其[寫入規則](job-database.md#421-寫入規則)寫入資料庫）
...
- 輸出：
  - `output/104/` 下同名的 CSV 與 JSON（見 [§6.2.1](#621-輸出檔)），欄位依 [job-database 的職缺欄位契約](job-database.md#621-職缺欄位契約)。
...
- Then：
  ...
  - 欄位順序等於[職缺欄位契約](job-database.md#621-職缺欄位契約)
- 通過條件：全部符合。
...
- FR-output-files：輸出 CSV（`utf-8-sig` 編碼）與 JSON（UTF-8）到 `output/104/`。
  - 欄位順序固定依[職缺欄位契約](job-database.md#621-職缺欄位契約)。
...
執行紀錄的欄位見 [job-database §6.2.2](job-database.md#622-保存的資訊)，本功能寫入時填入的值與額外記錄的條件欄位：
...
- 記錄的值與條件（欄位見 [job-database §6.2.2](job-database.md#622-保存的資訊)）：
  - `來源`：`匯入`
