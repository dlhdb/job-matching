...
- 使用者故事，一個故事一章，細節見各章的「需求」：
  ...
  - [預覽抓到的職缺，再決定要不要存進職缺資料庫](#6-預覽抓到的職缺再決定要不要存進職缺資料庫store)（`store`）：✅
...
- FR-search-form：在抓取頁的表單填搜尋條件後開始抓取，欄位見 [§4.2.2](#422-搜尋條件)。
  - 記住上次用的條件，下次打開抓取頁時沿用。
- FR-search-keyword：多個關鍵字可用半形或全形逗號分隔，逐一搜尋。
  - 重複的關鍵字只搜尋一次，保留首次出現的順序。
- FR-search-area：縣市從清單單選，轉成 104 的地區代碼。
  - 不選時搜尋全台灣。
- FR-search-dedup：單次抓取內，以 `職缺代碼` 去重，抓到的每筆職缺唯一。
- FR-search-request：單一請求失敗或逾時，不中斷整次抓取，規則見 [§4.2.3](#423-請求與失敗處理)。
  - 請求之間的延遲見 [§7](#7-非功能需求)。
- FR-search-progress：抓取中顯示進度，內容見 [§4.2.5](#425-抓取中與停止)。
- FR-search-stop：抓取中可以停止，規則見 [§4.2.5](#425-抓取中與停止)。
  - 不再發出新的請求。
  - 已經取完內容的職缺進入預覽。
...
流程從抓取頁的搜尋條件開始，抓完後在抓取頁預覽；存入見 [§6](#6-預覽抓到的職缺再決定要不要存進職缺資料庫store)。
...
- FR-job：抓取是在伺服器上跑的作業，規則見 [§8.2.1](#821-作業)。
