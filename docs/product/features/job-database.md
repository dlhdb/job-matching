...
- 使用者故事，一個故事一章，細節見各章的「需求」：
  ...
  - [在職缺表瀏覽、篩選累積下來的職缺](#5-在職缺表瀏覽篩選累積下來的職缺query)（`query`）：✅
  - [只看剛存入的職缺](#6-只看剛存入的職缺just-saved)（`just-saved`）：✅
...
- 保留職缺內容的歷史版本：只保留最新一版
- 在職缺表挑選過去任何一次寫入的職缺：只做「剛存入的職缺」（見 [§6](#6-只看剛存入的職缺just-saved)），職缺表才不依賴執行紀錄；執行紀錄的用途等趨勢分析規劃時再定（見 [待辦：做趨勢圖表](../../../backlog/tasks/trend-charts.md)）
- 在網頁上手動新增一筆職缺：和職缺欄位契約的平台中立化一起規劃，見 [TODO.md](../../../backlog/TODO.md#把職缺欄位契約改成平台中立)
...
以一次寫入的 `執行時間` `t`（見 [§8.2.2](#822-保存的資訊)）寫入一批職缺，對每筆職缺：
...
- 地區：比對 `地區` 是否包含輸入的文字，可以篩到縣市或行政區，例如 `台北市`、`內湖`。
  ...
  - `地區` 合併了縣市行政區與街道地址，所以也會比對到街道名稱；拆成兩欄後改成只比對縣市行政區（見 [TODO.md](../../../backlog/TODO.md#拆開地區欄位)）。
- 關鍵字與地區用同一套比對規則：
...
- When：
  ...
  - (c) 取消勾選到只剩一欄，再取消最後一欄
- Then：
  - (a) 顯示 [§5.2.2](#522-欄位) 的預設欄位，沒有顯示其他職缺欄位契約的欄位與 `首次出現時間`；可以選的欄位包含職缺欄位契約的全部欄位與兩個出現時間
...
- NFR-migration：資料庫結構改版時，保留既有的職缺、執行紀錄與每次寫入出現的職缺。
...
- 欄位名稱用中文，寫入與讀取都用同一組名稱。
- 寫進資料庫的職缺要照這份契約整理欄位，下游讀取職缺時也以它作為輸入契約。
- 目前的欄位來自 104 職缺，新增其他求職平台時在這裡擴充（見 [TODO.md](../../../backlog/TODO.md#把職缺欄位契約改成平台中立)）。
...
- `執行編號`：自動遞增的編號
- `執行時間`：這批職缺出現的時間，由來源功能提供，首次與最後出現時間都以它為準
- `來源`：寫入方式的名稱，由來源功能提供
  - 舊資料中的 `匯入`，代表從過去抓好的 JSON 檔匯入
- `職缺數`：這次寫入的職缺筆數
- 來源功能提供的寫入條件：搜尋條件（`關鍵字`、`地區`、`職缺性質`、`頁數`），意義由提供的功能定義。
  - 沒有提供該欄位的來源寫入時為 `null`。
  - 目前的欄位來自 104-job-scraper 的抓取，新增其他來源時要重新設計（見 [TODO.md](../../../backlog/TODO.md#把職缺欄位契約改成平台中立)）。
- `來源檔`：舊的匯入紀錄匯入的 JSON 檔名（不含目錄），其他紀錄為 `null`
