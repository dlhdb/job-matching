...
- 保存與查詢評分：屬 [job-score-database](job-score-database.md)，本功能只提供資料庫
- job-auto-scoring 改從資料庫讀取職缺：見 [TODO.md](../../../TODO.md#職缺自動評分範圍外)
...
- 輸出：職缺資料庫，保存的資訊見 [§7.2.2](#722-保存的資訊)。
  - job-auto-scoring、job-score-database 與 trend-analysis 需要以職缺代碼為準、欄位固定的職缺資料。
...
- 寫進資料庫的職缺要照這份契約整理欄位，job-auto-scoring 也以它作為輸入契約。
