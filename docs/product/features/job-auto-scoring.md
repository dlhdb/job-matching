...
- 狀態：待規劃
- 依賴：
  - job-database：從職缺資料庫讀取要評分的職缺，欄位依其職缺欄位契約〔規劃中〕
...
- 輸入：使用者提供的工作偏好與工作經歷
- 對職缺資料庫中還沒評分的職缺，或使用者挑的職缺，逐筆做多維度評分〔規劃中〕
- 產出總分與簡短評語，寫進評分資料庫（[job-score-database](job-score-database.md)）〔規劃中〕
...
- 使用者故事，一個故事一章，細節見各章的「需求」：
  ...
  - [一次評完多筆職缺](#6-一次評完多筆職缺batch)（`batch`）：〔規劃中〕
  - [評分結果存進評分資料庫](#7-評分結果存進評分資料庫store)（`store`）：✅
  - [重跑時不重複付 AI 費用](#8-重跑時不重複付-ai-費用cache)（`cache`）：〔規劃中移除〕
...
- 試跑結果檔的排序與篩選：只產出檔案，交給讀取結果檔的工具，例如 pandas〔規劃中〕
- Gemini 以外的 AI 供應商：只保留可替換的介面
- 通勤便利度、資歷門檻、工作型態與福利、競爭程度／新鮮度等維度：候選做法見 [TODO.md](../../../TODO.md)
- 修改爬蟲或擴充爬蟲欄位：見 [TODO.md](../../../TODO.md#爬蟲)
- 同一次執行內的失敗重試：失敗的職缺下次執行會再評（見 [§6](#6-一次評完多筆職缺batch)），立即重試見 [TODO.md](../../../TODO.md#職缺自動評分範圍外)〔規劃中〕
- 平行呼叫 AI：整批維持循序執行

## 3. 輸入與輸出

- 輸入的職缺：職缺資料庫中的職缺，以 `--db` 指定資料庫，與評分結果寫入的是同一個資料庫。〔規劃中〕
...
- 輸入的個人資料：`profile/preferences.yaml` 與 `profile/experience.md`，格式見 [§4.2.1](#421-個人資料檔)。
  - 由使用者自己維護，不來自其他功能。
- 輸入的上次評分：資料庫中同一筆職缺上次的 AI 維度與評語，用於沿用（見 [§8.2.1](#821-快取鍵)）。〔規劃中移除〕
- 終端機輸出：不論評幾筆，都只顯示進度與摘要，見 [§6.2.3](#623-摘要)。〔規劃中〕
- 試跑輸出：不論評幾筆，都寫成 `output/scores/` 下的試跑結果檔，見 [§9.2.2](#922-試跑結果檔)。〔規劃中〕
- 評分結果都寫進評分資料庫，寫入的欄位見 [§7.2.1](#721-寫入的評分紀錄)，查詢見 [job-score-database §4](job-score-database.md#4-依分數查詢評分query)，列表多帶的欄位見 [§10](#10-依分數列出時看得出是哪個模型評的list)。試跑的結果不寫入。

## 4. 評單筆職缺並看懂每個分數（score）

身為求職者，我想要依照寫成檔案的偏好與經歷替職缺評分，並看到每個維度的分數與理由，以便每次都用同一套標準，並知道總分是怎麼來的、判斷要不要相信它。

調整提示詞的開發者也用 `--job-no` 指定幾筆職缺反覆評分，並加上 `--dry-run`（`--job-no ... --dry-run`，見 [§14.2.2](#1422-cli)）避免寫進正式的評分。〔規劃中〕
...
- 「一筆職缺」是職缺資料庫中的一筆職缺〔規劃中〕，欄位見 [job-database 的職缺欄位契約](job-database.md#721-職缺欄位契約)。
...
- 回應不符合這個格式時（例如分數超出範圍），視為該次評分失敗：
  - 記下失敗原因，列在摘要中，繼續評下一筆（見 [§6](#6-一次評完多筆職缺batch)）。〔規劃中〕
  - 不自行修正分數。
...
- Given：
  ...
  - 固定的擬真測試資料：偏好檔、經歷檔，以及 5 筆改寫自 104 的職缺，已匯入測試用的職缺資料庫〔規劃中〕
- When：以 `--job-no` 指定其中第一筆有工作內容、而且沒被淘汰的職缺評分〔規劃中〕
- Then：
  - 從資料庫取出這筆的評分明細（見 [§7](#7-評分結果存進評分資料庫store)），符合 [§14.2.1](#1421-評分結果格式)，四個維度依序出現〔規劃中〕
...
- 評分結果（[§14.2.1](#1421-評分結果格式)）、試跑結果檔（[§9.2.2](#922-試跑結果檔)）與評分資料庫（[§7.2.1](#721-寫入的評分紀錄)）的 `評語` 用同一段文字。〔規劃中〕
...
- Then：
  - 正常結束
  - 資料庫中這筆的評分明細 `淘汰` 為 true、`淘汰原因` 不為空〔規劃中〕
- 通過條件：全部符合。
...
- 不指定 `--job-no`：
  - 評職缺資料庫中所有沒有任何評分紀錄的職缺。
    - 已經有評分的職缺不評，不論是自動或手動評分。所以手動評分不會被整批評分蓋過（查詢以最新的一筆為準，見 [§12](#12-同一筆職缺有多筆評分時查詢以代表的評分為準current)）。
...
- 上次送給 AI 的內容，用來判斷能否沿用（見 [§8.2.1](#821-快取鍵)）。〔規劃中移除〕
...
每筆職缺套用以下流程，取代 [§4.2.3](#423-評分流程) 的直接評分。沒被淘汰的職缺每次都呼叫 AI，不沿用上次的評分。〔規劃中〕
...
- 每筆評完就寫入資料庫。
  - 寫入是新增一筆，不覆寫任何紀錄（見 [§11](#11-保留評分歷史並和手動評分並存history)）。〔規劃中〕
  ...
  - 下次不指定 `--job-no` 執行時，已經有評分的職缺不再評（見 [§6.2.1](#621-要評哪些職缺)），只評剩下的職缺。〔規劃中〕
- 評分中途無法寫入資料庫時，顯示錯誤訊息並中止，已寫入的職缺留在資料庫。〔規劃中〕
...
- Given：
  ...
  - AI 的回應以固定內容代替
- When：以 `--job-no` 指定這三筆評分。已經有評分的職缺，不指定 `--job-no` 時不會評，所以要指定〔規劃中〕
- Then：
...
- Given：與 [AC-score-real](#ac-score-real真實評分-需網路)、[AC-batch-real](#ac-batch-real真實整批評分-需網路) 相同，兩者共用一個測試用的資料庫，開始前清空，再匯入測試職缺〔規劃中〕
- When：執行 AC-score-real 與 AC-batch-real 的評分〔規劃中〕
- Then：
  - AC-score-real：資料庫有該職缺的結果〔規劃中〕
  - AC-batch-real：評分成功與被淘汰的職缺都有結果〔規劃中〕
    - 評分失敗的職缺不檢查：共用的資料庫中可能留有 AC-score-real 寫入的結果。失敗不寫入由 [AC-store-write](#ac-store-write評分結果入庫) 驗證〔規劃中〕
  - 沒被淘汰的職缺：
    - 總分、評語與評分明細中的值一致〔規劃中〕
    - 供應商、模型為 `gemini`、`gemini-3.8-flash`
  - 被淘汰的職缺：總分、供應商、模型為 `null`，`評分明細` 的 `淘汰原因` 不是空清單〔規劃中〕
  - 印出資料庫的路徑，跑完保留，使用者可以直接打開查看
- 通過條件：全部符合。

## 8. 重跑時不重複付 AI 費用（cache）

〔規劃中移除〕改成沒被淘汰的職缺每次評分都呼叫 AI，本章在實作完成時刪除，理由見[決策紀錄：自動評分不做快取](../decisions/no-score-cache.md)。
...
- AI 的輸出完全由送出去的東西決定，所以快取鍵由以下四項算出，四項都相同才沿用：
  ...
  3. 給 AI 的指示
  4. 送給 AI 的資料
- 為什麼不把整份偏好檔放進快取鍵，見 [決策紀錄：自動評分不做快取](../decisions/no-score-cache.md)。
...
- 快取鍵不包含程式：
  ...
  - 修改 AI 回應的解讀方式後不會重問 AI。不提供清掉評分的方式（見 [§11](#11-保留評分歷史並和手動評分並存history)），要重問 AI 時改動快取鍵的來源。
...
- FR-dry-run-output：試跑結果寫成 JSON 與 CSV 兩個檔案，位置與格式依 [§9.2.2](#922-試跑結果檔)。
...
- 每筆除了 [§14.2.1](#1421-評分結果格式) 的欄位，還有：
...
- Then：
  - (a)
    ...
    - JSON 可解析，每個元素的鍵與順序符合 [§9.2.2](#922-試跑結果檔) 的欄位
    - CSV：
      - 表頭符合 [§9.2.2](#922-試跑結果檔) 的欄位順序
      - 列的順序與 JSON 相同
      - 未知維度的分數欄是空的
      - 被淘汰那列的 `淘汰原因` 以 `, ` 合併
...
- FR-history-append：自動評分每次寫入都新增一筆紀錄，不覆寫任何紀錄，規則見 [§11.2.2](#1122-什麼時候新增一筆)。
...
- FR-history-rename：`評分結果` 欄位改名為 `評分明細`，內容不變。
  ...
  - 評分結果格式（[§14.2.1](#1421-評分結果格式)）指一筆職缺評分結果的整份 JSON，名稱不變。
...
- NFR-cost：只有真的需要 AI 判斷時才呼叫 AI。
  ...
  - 送給 AI 的內容沒變的職缺，沿用上次的 AI 評分（見 [§8](#8-重跑時不重複付-ai-費用cache)）。〔規劃中移除〕
...
- NFR-cost：
  ...
  - [AC-cache](#ac-cache沿用上次的-ai-評分)〔規劃中移除〕
...
- FR-cli：提供 `src/score_job.py` CLI，參數與錯誤訊息依 [§14.2.2](#1422-cli)。
  - 以 `--job-no` 指定要評的職缺，省略時評所有還沒評分的職缺（見 [§6.2.1](#621-要評哪些職缺)）；不論評幾筆，都只印出進度與摘要。〔規劃中〕
...
- 試跑結果檔（[§9.2.2](#922-試跑結果檔)）與寫進評分資料庫的 `評分明細` 欄位（[§7.2.1](#721-寫入的評分紀錄)）都以它為基礎。〔規劃中〕
- 不在終端機印出，要看時從資料庫取出評分明細（見 [§7](#7-評分結果存進評分資料庫store)）。〔規劃中〕
- 使用中文鍵名，與爬蟲的輸出一致。
...
- `--job-no`：只評這幾筆職缺，可以指定多筆，以空白分隔〔規劃中〕
...
- `--dry-run`：試跑（見 [§9](#9-換設定試跑而不影響正式分數dry-run)），用於換一份偏好檔、經歷或模型，實際看 AI 會給幾分、理由是什麼
  - 完整評分，會呼叫 AI
  - 必須搭配 `--job-no`〔規劃中〕
  - 只從資料庫讀職缺，不讀寫評分，不影響已存的正式分數〔規劃中〕
  - 結果都寫成試跑結果檔（檔名見 [§9.2.2](#922-試跑結果檔)），不印出評分結果的 JSON〔規劃中〕
- `--db`：讀取職缺與寫入評分的資料庫，預設為專案根目錄的 `data/jobs.db`（見 [§7](#7-評分結果存進評分資料庫store)）〔規劃中〕
  - 檔案不存在時顯示錯誤訊息並結束，不建立資料庫檔

輸出：

- 正式評分：結果只寫進資料庫，終端機只顯示進度與摘要。〔規劃中〕
- 試跑：結果寫成試跑結果檔，終端機只顯示進度與摘要。〔規劃中〕

以下情況會顯示錯誤訊息並結束，不產生評分結果：

- 個人資料檔缺少或格式錯誤
- 資料庫檔不存在〔規劃中〕
- `--job-no` 指定的職缺代碼不在職缺資料庫〔規劃中〕
- 試跑沒有指定 `--job-no`〔規劃中〕
- 不支援的供應商
- 缺少 API key（只有真的要呼叫 AI 時才需要）
- 無法開啟或寫入資料庫（評分中途無法寫入的行為見 [§7.2.2](#722-流程)）

個別職缺評分失敗（AI 呼叫失敗，或回應不通過 [§4.2.9](#429-ai-回應的檢查) 的檢查）不算錯誤，失敗筆數見摘要。〔規劃中〕
...
- When：
  ...
  - (b) 指定不存在的職缺代碼
  - (c) 指定不支援的供應商
  - (d) 以 `--db` 指定不存在的資料庫檔〔規劃中〕
- Then：
  - (a) 顯示提到 `GEMINI_API_KEY` 的錯誤訊息並結束
  - (b) 顯示錯誤訊息並結束
  - (c) 被拒絕，提示不支援的供應商
  - (d) 顯示錯誤訊息並結束，沒有建立資料庫檔〔規劃中〕
- 通過條件：全部符合。
