...
- FR-history-coexist：手動評分與自動評分並存。
  ...
  - 自動評分不覆寫手動評分。
- FR-history-comment：被淘汰的職缺，`評語` 改為淘汰原因組成的文字，格式見 [§11.2.3](#1123-淘汰時的評語)。
...
- 格式是 `淘汰：` 接上[淘汰原因](#521-硬性淘汰規則)，多個原因以 `；` 分隔。
  - 例如 `淘汰：職稱含排除關鍵字：實習；薪資上限 30000 低於底線 40000`。
- 評分結果（[§14.2.1](#1421-評分結果格式)）與整批結果檔（[§6.2.2](#622-結果檔)）的 `評語` 用同一段文字。
...
- `評語`（str | null）：AI 產生的一到兩句總評，程式不修改
  - 被淘汰時為 `null`
  - 〔規劃中〕被淘汰時改為淘汰原因組成的評語，型態改為 str（見 [§11](#11-保留評分歷史並和手動評分並存history)）
