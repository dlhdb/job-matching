...
- 對 AI 的評分表態（同意／不同意）與已讀、想投、略過等標記：屬之後的標記功能，另外保存，不共用評分紀錄，見 [TODO.md](../../../TODO.md#標記職缺)
- 標出職缺內容或設定改變後過時的評分：見 [TODO.md](../../../TODO.md#標出過時的評分)
...
- 同一次評分內的失敗重試：失敗的職缺可以再勾選重評（見 [§4.2.14](#4214-評分中與評完)），立即重試見 [TODO.md](../../../TODO.md#評分失敗時立即重試)
- 平行呼叫 AI：整批維持循序執行
...
每筆職缺照 [§4.2.3](#423-評分流程) 評分後，把結果寫入資料庫。沒被淘汰的職缺每次都呼叫 AI，不沿用上次的評分（見[決策紀錄：自動評分不做快取](../decisions/no-score-cache.md)）。
...
- 要不要重評由使用者在確認視窗決定，不做快取（見[決策紀錄：自動評分不做快取](../decisions/no-score-cache.md)）。
...
- 同一筆職缺有多筆評分紀錄時，用評分時間最新的一筆。
  - 評分時間相同時，用後寫入的一筆。
  - 理由見[決策紀錄：多筆評分以最新的一筆為準](../decisions/latest-score-wins.md)。
