...
- 使用者故事，一個故事一章，細節見各章的「需求」：
  - [自動評分並看懂每個分數](#4-自動評分並看懂每個分數score)（`score`）：✅
  - [明顯不合的職缺直接淘汰](#5-明顯不合的職缺直接淘汰filter)（`filter`）：✅
  - [換了設定後重評指定的職缺](#6-換了設定後重評指定的職缺rescore)（`rescore`）：✅
  - [換設定試跑而不影響正式分數](#7-換設定試跑而不影響正式分數dry-run)（`dry-run`）：✅
  - [追蹤同一筆職缺的評分變化](#8-追蹤同一筆職缺的評分變化history)（`history`）：✅
- [§9 非功能需求](#9-非功能需求)：只在必要時呼叫 AI、個人資料與 API key 不進版控、資訊不足時分數為 `null`
- [§10 共用規則](#10-共用規則)：評分結果格式、CLI、AI 供應商與 API key

範圍外（實作時不要做）：

- 依分數查詢評分、手動記錄評分：屬 [job-score-database](job-score-database.md)
- 互動式瀏覽介面：見 [TODO.md](../../../TODO.md#做出-web-app-第-2-階段)，介面形式見[決策紀錄：介面形式選型](../decisions/interface-selection.md)
- 試跑結果檔的排序與篩選：只產出檔案，交給讀取結果檔的工具，例如 pandas
- Gemini 以外的 AI 供應商：只保留可替換的介面
- 通勤便利度、資歷門檻、工作型態與福利、競爭程度／新鮮度等維度：候選做法見 [TODO.md](../../../TODO.md)
- 修改爬蟲或擴充爬蟲欄位：見 [TODO.md](../../../TODO.md#加入資歷門檻與工作型態維度)
- 同一次執行內的失敗重試：失敗的職缺下次執行會再評（見 [§4.2.12](#4212-要評哪些職缺)），立即重試見 [TODO.md](../../../TODO.md#評分失敗時立即重試)
- 平行呼叫 AI：整批維持循序執行

## 3. 輸入與輸出

- 輸入的職缺：職缺資料庫中的職缺，以 `--db` 指定資料庫，與評分結果寫入的是同一個資料庫。
  - 要評哪些職缺：不指定 `--job-no` 時見 [§4.2.12](#4212-要評哪些職缺)，指定時見 [§6.2.1](#621-要評哪些職缺)。
...
- 輸入的個人資料：`profile/preferences.yaml` 與 `profile/experience.md`，格式見 [§4.2.1](#421-個人資料檔)。
  - 由使用者自己維護，不來自其他功能。
- 終端機輸出：不論評幾筆，都只顯示進度與摘要，見 [§4.2.14](#4214-摘要)。
- 試跑輸出：不論評幾筆，都寫成 `output/scores/` 下的試跑結果檔，見 [§7.2.2](#722-試跑結果檔)。
- 評分結果都寫進評分資料庫，寫入的欄位見 [§4.2.10](#4210-寫入的評分紀錄)，查詢見 [job-score-database §4](job-score-database.md#4-依分數查詢評分query)，列表多帶的欄位見 [§8.2.5](#825-列表多帶的欄位)。試跑的結果不寫入。

## 4. 自動評分並看懂每個分數（score）

身為求職者，我想要依照我的偏好與經歷，自動替還沒評過的職缺評分，並附上每個維度的分數與理由，以便不必逐筆閱讀就知道哪些職缺值得細看、為什麼。
...
- FR-score-store：評分結果都寫進評分資料庫，寫入的欄位依 [§4.2.10](#4210-寫入的評分紀錄)。
  - 評分成功與被淘汰的職缺都寫入。
  - 評分失敗的職缺不寫入。
- FR-score-db-path：CLI 以 `--db` 指定資料庫路徑。
  - 預設為 `data/jobs.db`，與 [job-database §8.1](job-database.md#81-需求) 的預設路徑相同。
  - 評分結果都會寫入資料庫，只有試跑例外（見 [§7](#7-換設定試跑而不影響正式分數dry-run)）。
- FR-score-get：以職缺代碼取出單筆的 `評分明細`（[§4.2.10](#4210-寫入的評分紀錄)），得到各維度的分數與理由。
  - 內容與寫入時的評分結果相同。
  - 沒有評過這筆職缺時明確表示查不到。
  - 這筆職缺只有手動評分（見 [job-score-database §5](job-score-database.md#5-手動記錄評分manual)）時，沒有 `評分明細`，同樣表示查不到。
- FR-score-select：評職缺資料庫中所有還沒評分的職缺，範圍與順序依 [§4.2.12](#4212-要評哪些職缺)。
  - 評分失敗的職缺沒有寫進資料庫，下次執行時會再評一次。
- FR-score-order：依 [§4.2.12](#4212-要評哪些職缺) 的順序，逐筆套用 [§4.2.11](#4211-評完寫入資料庫) 的流程。
- FR-score-failure：單筆評分失敗（AI 呼叫失敗或回應不通過 [§4.2.9](#429-ai-回應的檢查) 的檢查）時，不中斷整批。
  - 記下失敗原因並繼續下一筆。
  - 這樣整批已經花掉的時間與 AI 費用不會白費。
- FR-score-summary：整批評分結束時印出摘要，格式見 [§4.2.14](#4214-摘要)：
  - 總筆數、成功、淘汰、失敗筆數
  - 失敗的職缺與失敗原因
- FR-score-no-file：整批評分不寫結果檔，評分結果只寫進評分資料庫。

依分數列出評分屬 [job-score-database §4](job-score-database.md#4-依分數查詢評分query)，本功能只在列表多帶欄位（見 [§8.2.5](#825-列表多帶的欄位)）。
...
- `profile/experience.md`：工作經歷與技能，原文放進提示詞。
  - 格式是自由格式的 Markdown。
- 兩個檔案都不進版控，版控中只保留 `.example` 範本（見 [§9.1](#91-需求)）。
...
- 回應不符合這個格式時（例如分數超出範圍），視為該次評分失敗：
  - 記下失敗原因，列在摘要中，繼續評下一筆（見 [§4.2.13](#4213-整批評分流程)）。
  - 不自行修正分數。
...
- `職缺代碼`、`淘汰`、`總分`、`評語`：取自評分結果（[§10.2.1](#1021-評分結果格式)）的同名欄位。
- `評分時間`：這次寫入的時間。

另外疊加以下幾項，只有自動評分會有：

- `評分明細`：整份評分結果，格式依 [§10.2.1](#1021-評分結果格式)，要看各維度的分數與理由、淘汰原因時讀這裡。
  - 和 `總分`、`評語` 區分：`總分`、`評語` 是結論，`評分明細` 是得出結論的各維度分數與理由、淘汰原因。
  - 依分數列出時不帶這一項（見 [§8.2.5](#825-列表多帶的欄位)），以職缺代碼另外取出。
- `供應商`、`模型`：產生 AI 維度時使用的供應商與模型。
  - 被淘汰的職缺沒有呼叫 AI，這兩項為 `null`。

其他規則：

- 不記錄評分時使用的偏好檔與經歷內容，只記錄供應商與模型（已決定改成記錄設定的版本與送評時的職缺內容快照，見 [web app 原型的設計](../web-app-prototype.md#33-job-auto-scoring)）。

#### 4.2.11 評完寫入資料庫

每筆職缺照 [§4.2.3](#423-評分流程) 評分後，把結果寫入資料庫。沒被淘汰的職缺每次都呼叫 AI，不沿用上次的評分。
...
- 每筆評完就寫入資料庫。
  - 寫入是新增一筆，不覆寫任何紀錄，並和手動評分並存（見 [§8.2.2](#822-什麼時候新增一筆)）。
  - 整批中途中止時，已經評完的職缺留在資料庫。
  - 下次執行時，已經有評分的職缺不再評（見 [§4.2.12](#4212-要評哪些職缺)），只評剩下的職缺。
- 評分中途無法寫入資料庫時，顯示錯誤訊息並中止，已寫入的職缺留在資料庫。

#### 4.2.12 要評哪些職缺

- 評職缺資料庫中所有沒有任何評分紀錄的職缺。
  - 已經有評分的職缺不評，不論是自動或手動評分。所以手動評分不會被整批評分蓋過（查詢以最新的一筆為準，見 [§8.2.3](#823-代表的評分)）。
...
```mermaid
flowchart TD
    L["從職缺資料庫選出要評的職缺（§4.2.12）"] --> N{還有下一筆？}
    N -->|有| S["§4.2.11 評完寫入資料庫"]
    S -->|淘汰| R[記錄結果]
    S -->|成功| R
...
- 通過條件：
  - 評分成功，而且理由不是空的
  - 使用者取出這些職缺的評分明細（見 [§4.1](#41-需求)）閱讀，確認理由確實引用了職缺內容，分數與自己的判斷大致相符
...
- When：評這三筆
- Then：
  - 成功與淘汰的兩筆寫入資料庫，欄位依 [§4.2.10](#4210-寫入的評分紀錄)
  - 寫入的 `評分明細` 符合 [§10.2.1](#1021-評分結果格式)，淘汰、總分、評語與其中的值一致
  - 淘汰那筆的總分為 `null`
  - 評分失敗那筆沒有寫入
- 通過條件：全部符合。

#### AC-score-db-path：資料庫路徑
...
- Then：
  - 評過時得到完整的評分結果，與寫入時相同，格式符合 [§10.2.1](#1021-評分結果格式)，各維度的分數與理由都在
  - 沒評過與只有手動評分時都明確表示查不到
- 通過條件：全部符合。

#### AC-score-select：選出還沒評分的職缺
...
#### AC-score-cli：整批 CLI 與摘要

- Given：測試用的偏好、經歷，職缺資料庫中有一筆正常、一筆會被淘汰的職缺，都還沒評分
- When：
  - (a) 執行 CLI，AI 正常回應
  - (b) 沒有設定 API key，職缺資料庫中只有會被淘汰的那筆
  - (c) 沒有設定 API key
  - (d) 執行 CLI，正常那筆 AI 呼叫失敗
- Then：
...
- 評分結果（[§10.2.1](#1021-評分結果格式)）、試跑結果檔（[§7.2.2](#722-試跑結果檔)）與評分資料庫（[§4.2.10](#4210-寫入的評分紀錄)）的 `評語` 用同一段文字。
...
## 6. 換了設定後重評指定的職缺（rescore）

身為求職者，我想要指定幾筆職缺重新評分，評過的也照樣重評，以便換了偏好、經歷或模型之後，這幾筆的分數能反映新的設定。

### 6.1 需求

- FR-rescore：以 `--job-no` 指定職缺時，只評這幾筆，範圍與順序依 [§6.2.1](#621-要評哪些職缺)。
  - 已經評過的也照樣重評，不沿用上次的評分。
  - 重評失敗的職缺不寫入，保留原本的評分。
  - 指定一筆或多筆的行為相同。
  - 逐筆評分、單筆失敗不中斷、印出摘要、不寫結果檔，都和評所有還沒評過的職缺相同（見 [§4.2.13](#4213-整批評分流程)）。
...
- Given：
  - 一筆不會被淘汰、已經有自動評分的職缺
  - 這次重評時 AI 呼叫失敗
- When：以 `--job-no` 指定這筆重評
- Then：
  - 摘要列出這筆失敗
  - 資料庫中這筆原本的評分不變，沒有新增評分
- 通過條件：全部符合。

## 7. 換設定試跑而不影響正式分數（dry-run）

身為求職者，我想要換一份偏好、經歷或模型試評幾筆職缺，而且不影響已存的分數，以便比較不同設定的結果，再決定要不要換。

調整提示詞的開發者也用 `--job-no` 指定幾筆職缺反覆試跑（`--job-no ... --dry-run`，見 [§10.2.2](#1022-cli)），避免寫進正式的評分。
...
- FR-dry-run-output：試跑結果寫成 JSON 與 CSV 兩個檔案，位置與格式依 [§7.2.2](#722-試跑結果檔)。
...
- 流程、進度、摘要與單筆失敗的處理都依 [§4.2.13](#4213-整批評分流程) 與 [§4.2.14](#4214-摘要)，評分失敗時列在摘要中，不算錯誤。
- 開始評分前先印出 `[i] 試跑：不寫入資料庫，不影響已存的評分`。
- 資料庫只放正式評分：換偏好檔、經歷、模型或提示詞來比較時要加 `--dry-run`，否則會寫進資料庫，成為這筆職缺最新的評分。
- 摘要（[§4.2.14](#4214-摘要)）最後多印兩行試跑結果檔的路徑：
...
- 需要呼叫 AI 時才需要 API key，規則同 [§4.2.13](#4213-整批評分流程)；缺少 API key 時不寫試跑結果檔。
- 參數說明見 [§10.2.2](#1022-cli)。
...
- 每筆除了 [§10.2.1](#1021-評分結果格式) 的欄位，還有：
...
- `職缺代碼`（str）
- `職缺名稱`、`公司名稱`、`薪資待遇`、`職缺連結`（同 [job-database §8.2.1](job-database.md#821-職缺欄位契約)）：從職缺原樣帶入
- `淘汰`、`淘汰原因`、`維度`、`總分`、`未知維度`、`評語`（同 [§10.2.1](#1021-評分結果格式)）
...
- Then：
  - (a)
    ...
    - JSON 可解析，每個元素的鍵與順序符合 [§7.2.2](#722-試跑結果檔) 的欄位
    - CSV：
      - 表頭符合 [§7.2.2](#722-試跑結果檔) 的欄位順序
      - 列的順序與 JSON 相同
      - 未知維度的分數欄是空的
      - 被淘汰那列的 `淘汰原因` 以 `, ` 合併
...
## 8. 追蹤同一筆職缺的評分變化（history）

身為求職者，我想要同一筆職缺重評或手動評分後，之前的評分都還留著，並分得出每一筆是自己評的還是哪個模型評的，以便比較前後的判斷，也不怕一次錯誤的評分蓋掉原本的資料。

本章在評分資料庫的[評分紀錄契約](job-score-database.md#821-評分紀錄契約)、[保存規則](job-score-database.md#822-保存規則)與[依分數查詢評分](job-score-database.md#4-依分數查詢評分query)之上，疊加：

- 自動與手動評分每次都新增一筆，互不覆寫
- 同一筆職缺有多筆評分時，查詢以代表的評分為準，也能取出所有評分紀錄
- 列表多帶產生評分的供應商與模型

### 8.1 需求

- FR-history-source：每筆評分紀錄標示 `評分來源`，值依 [§8.2.1](#821-評分來源)。
  - 不遷移既有的評分紀錄：本功能之前建立的職缺資料庫要刪除後重建，評分紀錄不保留（遷移見 [TODO.md](../../../TODO.md#補上評分紀錄的遷移)）。
- FR-history-append：自動評分每次寫入都新增一筆紀錄，不覆寫任何紀錄，規則見 [§8.2.2](#822-什麼時候新增一筆)。
...
- FR-history-current：以下查詢每筆職缺只用一筆代表的評分，選法依 [§8.2.3](#823-代表的評分)。
  ...
  - 以職缺代碼取出評分明細（[§4.1](#41-需求)）
  - 列表與取出的評分紀錄多帶 `評分來源` 欄位。
  - 代表的評分是手動評分時沒有評分明細，取出評分明細時明確表示查不到。
- FR-history-all：以職缺代碼取出這筆職缺的所有評分紀錄，依評分時間由新到舊，評分時間相同時後寫入的在前。
  - 沒有評過這筆職缺時是空清單。
- FR-history-list：依分數列出評分時，每筆在評分資料庫列表的欄位之外，多帶 [§8.2.5](#825-列表多帶的欄位) 的欄位。
...
- `評分來源`（[§8.2.1](#821-評分來源)）
- `供應商`、`模型`（[§4.2.10](#4210-寫入的評分紀錄)）
- `評分明細`（[§4.2.10](#4210-寫入的評分紀錄)）：用來比較 AI 每一次各維度的分數與理由
  - 手動評分沒有評分明細，為 `null`

#### 8.2.5 列表多帶的欄位

- `供應商`、`模型`：同 [§4.2.10](#4210-寫入的評分紀錄)，被淘汰的職缺為 `null`。
- 不帶 `評分明細`：要看各維度的分數與理由時，以職缺代碼另外取出（見 [§4.1](#41-需求)）。
...
#### AC-history-all：所有評分紀錄

- Given：與 [AC-history-current](#ac-history-current代表的評分) 相同
...
- NFR-privacy：真實的個人資料檔與 API key 不進版控。版控中只提供範本：
  ...
  - `.env` 的範本是 `.env.example`（見 [§10.2.3](#1023-ai-供應商與-api-key)）。
...
- 整批依序呼叫 AI，不平行呼叫（見 [§4.2.13](#4213-整批評分流程)）。
...
- NFR-cost：
  ...
  - [AC-score-order](#ac-score-order整批評分)（被淘汰的職缺沒有呼叫）
...
- FR-cli：提供 `src/score_job.py` CLI，參數與錯誤訊息依 [§10.2.2](#1022-cli)。
  - 以 `--job-no` 指定要評的職缺（見 [§6](#6-換了設定後重評指定的職缺rescore)），省略時評所有還沒評分的職缺（見 [§4](#4-自動評分並看懂每個分數score)）；不論評幾筆，都只印出進度與摘要。
  - `--dry-run` 試跑，見 [§7](#7-換設定試跑而不影響正式分數dry-run)。
- FR-llm：AI 供應商以 `--provider` 選擇，規則依 [§10.2.3](#1023-ai-供應商與-api-key)。
  - 預設使用 Gemini。
...
- 試跑結果檔（[§7.2.2](#722-試跑結果檔)）與寫進評分資料庫的 `評分明細` 欄位（[§4.2.10](#4210-寫入的評分紀錄)）都以它為基礎。
- 不在終端機印出，要看時從資料庫取出評分明細（見 [§4](#4-自動評分並看懂每個分數score)）。
- 使用中文鍵名，與爬蟲的輸出一致。
...
- `--job-no`：只評這幾筆職缺，可以指定多筆，以空白分隔
  - 指定一筆或多筆的行為相同，依指定的順序評，已經評過的也照樣重評（見 [§6](#6-換了設定後重評指定的職缺rescore)）
  - 有職缺代碼不在職缺資料庫時顯示錯誤訊息並結束，一筆都不評
  - 省略時評所有還沒評分的職缺（見 [§4](#4-自動評分並看懂每個分數score)）
...
- `--dry-run`：試跑（見 [§7](#7-換設定試跑而不影響正式分數dry-run)），用於換一份偏好檔、經歷或模型，實際看 AI 會給幾分、理由是什麼
  - 完整評分，會呼叫 AI
  - 必須搭配 `--job-no`
  - 只從資料庫讀職缺，不讀寫評分，不影響已存的正式分數
  - 結果都寫成試跑結果檔（檔名見 [§7.2.2](#722-試跑結果檔)），不印出評分結果的 JSON
- `--db`：讀取職缺與寫入評分的資料庫，預設為專案根目錄的 `data/jobs.db`（見 [§4](#4-自動評分並看懂每個分數score)）
...
- 無法開啟或寫入資料庫（評分中途無法寫入的行為見 [§4.2.11](#4211-評完寫入資料庫)）

個別職缺評分失敗（AI 呼叫失敗，或回應不通過 [§4.2.9](#429-ai-回應的檢查) 的檢查）不算錯誤，失敗筆數見摘要。

#### 10.2.3 AI 供應商與 API key
