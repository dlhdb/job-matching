...
- 使用者故事，一個故事一章，細節見各章的「需求」：
  - [依偏好與經歷評分並看懂每個分數](#4-依偏好與經歷評分並看懂每個分數score)（`score`）：✅
  - [明顯不合的職缺直接淘汰](#5-明顯不合的職缺直接淘汰filter)（`filter`）：✅
  - [評完所有還沒評分的職缺](#6-評完所有還沒評分的職缺unscored)（`unscored`）：✅
  - [重評指定的職缺](#7-重評指定的職缺rescore)（`rescore`）：✅
  - [評分結果存進評分資料庫](#8-評分結果存進評分資料庫store)（`store`）：✅
...
- 同一次執行內的失敗重試：失敗的職缺下次執行會再評（見 [§6](#6-評完所有還沒評分的職缺unscored)），立即重試見 [TODO.md](../../../TODO.md#職缺自動評分範圍外)
- 平行呼叫 AI：整批維持循序執行

## 3. 輸入與輸出

- 輸入的職缺：職缺資料庫中的職缺，以 `--db` 指定資料庫，與評分結果寫入的是同一個資料庫。
  - 要評哪些職缺：不指定 `--job-no` 時見 [§6.2.1](#621-要評哪些職缺)，指定時見 [§7.2.1](#721-要評哪些職缺)。
...
- 試跑輸出：不論評幾筆，都寫成 `output/scores/` 下的試跑結果檔，見 [§9.2.2](#922-試跑結果檔)。
- 評分結果都寫進評分資料庫，寫入的欄位見 [§8.2.1](#821-寫入的評分紀錄)，查詢見 [job-score-database §4](job-score-database.md#4-依分數查詢評分query)，列表多帶的欄位見 [§10](#10-依分數列出時看得出是哪個模型評的list)。試跑的結果不寫入。

## 4. 依偏好與經歷評分並看懂每個分數（score）
...
- 回應不符合這個格式時（例如分數超出範圍），視為該次評分失敗：
  - 記下失敗原因，列在摘要中，繼續評下一筆（見 [§6](#6-評完所有還沒評分的職缺unscored)）。
  - 不自行修正分數。
...
- Then：
  - 從資料庫取出這筆的評分明細（見 [§8](#8-評分結果存進評分資料庫store)），符合 [§14.2.1](#1421-評分結果格式)，四個維度依序出現
...
- 評分結果（[§14.2.1](#1421-評分結果格式)）、試跑結果檔（[§9.2.2](#922-試跑結果檔)）與評分資料庫（[§8.2.1](#821-寫入的評分紀錄)）的 `評語` 用同一段文字。
...
## 6. 評完所有還沒評分的職缺（unscored）

身為求職者，我想要一次評完所有還沒評分的職缺，並在結束時看到成功、淘汰、失敗各幾筆，以便不必自己挑出要評的職缺，也知道哪些職缺評分失敗。

### 6.1 需求

- FR-unscored-select：不指定 `--job-no` 時，評職缺資料庫中所有還沒評分的職缺，範圍與順序依 [§6.2.1](#621-要評哪些職缺)。
  - 評分失敗的職缺不會寫進資料庫（見 [§8](#8-評分結果存進評分資料庫store)），下次執行時會再評一次。
- FR-unscored-order：依 [§6.2.1](#621-要評哪些職缺) 的順序，逐筆套用 [§4.2.3](#423-評分流程) 的流程。
- FR-unscored-failure：單筆評分失敗（AI 呼叫失敗或回應不通過 [§4.2.9](#429-ai-回應的檢查) 的檢查）時，不中斷整批。
  - 記下失敗原因並繼續下一筆。
  - 這樣整批已經花掉的時間與 AI 費用不會白費。
- FR-unscored-summary：整批評分結束時印出摘要，格式見 [§6.2.3](#623-摘要)：
  - 總筆數、成功、淘汰、失敗筆數
  - 失敗的職缺與失敗原因
- FR-unscored-no-file：整批評分不寫結果檔，評分結果只寫進評分資料庫（見 [§8](#8-評分結果存進評分資料庫store)）。
...
- 評職缺資料庫中所有沒有任何評分紀錄的職缺。
  ...
  - 換了偏好、經歷或模型之後，要重評某幾筆時用 `--job-no` 指定（見 [§7](#7-重評指定的職缺rescore)）。
...
- 每筆的「§4.2.3 評分流程」改為 [§8.2.2](#822-流程)：評完寫入資料庫。
- 逐筆處理，不平行呼叫 AI。
...
- Then：
  - 正常結束
  - 印出摘要與前幾名的評語供使用者閱讀
- 只驗真實串接：選出哪些職缺、寫進資料庫等業務規則，由 [AC-unscored-select](#ac-unscored-select選出還沒評分的職缺)、[AC-store-write](#ac-store-write評分結果入庫) 以固定的 AI 回應驗證。
- 通過條件：
  - 自動檢查全部符合
  - 使用者抽查前幾名的評分理由是否合理
...
身為求職者，我想要指定幾筆職缺重新評分，已經評過的也照樣重評，以便換了偏好、經歷或模型之後，更新這幾筆的分數。

### 7.1 需求

- FR-rescore：以 `--job-no` 指定職缺時，只評這幾筆，範圍與順序依 [§7.2.1](#721-要評哪些職缺)。
  - 已經評過的也照樣重評，不沿用上次的評分。
  - 指定一筆或多筆的行為相同。
  - 逐筆評分、單筆失敗不中斷、印出摘要、不寫結果檔，都和評完所有還沒評分的職缺相同（見 [§6](#6-評完所有還沒評分的職缺unscored)）。
...
- FR-store-write：評分結果都寫進評分資料庫，寫入的欄位依 [§8.2.1](#821-寫入的評分紀錄)。
  - 評分成功與被淘汰的職缺都寫入。
  - 評分失敗的職缺不寫入，保留原本的評分。
...
- FR-store-get：以職缺代碼取出單筆的 `評分明細`（[§8.2.1](#821-寫入的評分紀錄)），得到各維度的分數與理由。
  - 內容與寫入時的評分結果相同。
  - 沒有評過這筆職缺時明確表示查不到。
...
- 每筆評完就寫入資料庫。
  - 寫入是新增一筆，不覆寫任何紀錄（見 [§11](#11-保留評分歷史並和手動評分並存history)）。
...
- Then：
  - 成功與淘汰的兩筆寫入資料庫，欄位依 [§8.2.1](#821-寫入的評分紀錄)
...
- Given：與 [AC-score-real](#ac-score-real真實評分-需網路)、[AC-unscored-real](#ac-unscored-real真實整批評分-需網路) 相同，兩者共用一個測試用的資料庫，開始前清空，再匯入測試職缺
- When：執行 AC-score-real 與 AC-unscored-real 的評分
- Then：
  - AC-score-real：資料庫有該職缺的結果
  - AC-unscored-real：評分成功與被淘汰的職缺都有結果
...
調整提示詞的開發者也用 `--job-no` 指定幾筆職缺反覆試跑（`--job-no ... --dry-run`，見 [§14.2.2](#1422-cli)），避免寫進正式的評分。
...
- 要評的職缺：`--job-no` 指定的職缺，找不到職缺與重複指定的處理同 [§7.2.1](#721-要評哪些職缺)。
...
- `供應商`、`模型`：同 [§8.2.1](#821-寫入的評分紀錄)，被淘汰的職缺為 `null`。
- 不帶 `評分明細`：要看各維度的分數與理由時，以職缺代碼另外取出（見 [§8](#8-評分結果存進評分資料庫store)）。
...
- FR-history-append：自動評分每次寫入都新增一筆紀錄，不覆寫任何紀錄，規則見 [§11.2.2](#1122-什麼時候新增一筆)。
  - 評分失敗時照 [§8](#8-評分結果存進評分資料庫store) 不寫入。
...
- 手動評分每次寫入也新增一筆。
- 評分失敗時不寫入（見 [§8](#8-評分結果存進評分資料庫store)）。
...
本章依 [§11](#11-保留評分歷史並和手動評分並存history) 留下的多筆評分紀錄，調整評分資料庫與 [§8](#8-評分結果存進評分資料庫store) 的查詢。
...
- FR-current-pick：以下查詢每筆職缺只用一筆代表的評分，選法依 [§12.2.1](#1221-代表的評分)。
  ...
  - 以職缺代碼取出評分明細（[§8](#8-評分結果存進評分資料庫store)）
...
- `供應商`、`模型`（[§8.2.1](#821-寫入的評分紀錄)）
- `評分明細`（[§8.2.1](#821-寫入的評分紀錄)）：用來比較 AI 每一次各維度的分數與理由
  - 手動評分沒有評分明細，為 `null`
...
- NFR-cost：
  ...
  - [AC-unscored-order](#ac-unscored-order整批評分)（被淘汰的職缺沒有呼叫）
...
- FR-cli：提供 `src/score_job.py` CLI，參數與錯誤訊息依 [§14.2.2](#1422-cli)。
  - 以 `--job-no` 指定要評的職缺（見 [§7](#7-重評指定的職缺rescore)），省略時評所有還沒評分的職缺（見 [§6](#6-評完所有還沒評分的職缺unscored)）；不論評幾筆，都只印出進度與摘要。
...
- 試跑結果檔（[§9.2.2](#922-試跑結果檔)）與寫進評分資料庫的 `評分明細` 欄位（[§8.2.1](#821-寫入的評分紀錄)）都以它為基礎。
- 不在終端機印出，要看時從資料庫取出評分明細（見 [§8](#8-評分結果存進評分資料庫store)）。
- 使用中文鍵名，與爬蟲的輸出一致。
...
- `--job-no`：只評這幾筆職缺，可以指定多筆，以空白分隔
  - 指定一筆或多筆的行為相同，依指定的順序評，已經評過的也照樣重評（見 [§7](#7-重評指定的職缺rescore)）
  - 有職缺代碼不在職缺資料庫時顯示錯誤訊息並結束，一筆都不評
  - 省略時評所有還沒評分的職缺（見 [§6](#6-評完所有還沒評分的職缺unscored)）
...
- `--dry-run`：試跑（見 [§9](#9-換設定試跑而不影響正式分數dry-run)），用於換一份偏好檔、經歷或模型，實際看 AI 會給幾分、理由是什麼
  ...
  - 結果都寫成試跑結果檔（檔名見 [§9.2.2](#922-試跑結果檔)），不印出評分結果的 JSON
- `--db`：讀取職缺與寫入評分的資料庫，預設為專案根目錄的 `data/jobs.db`（見 [§8](#8-評分結果存進評分資料庫store)）
...
- 無法開啟或寫入資料庫（評分中途無法寫入的行為見 [§8.2.2](#822-流程)）
