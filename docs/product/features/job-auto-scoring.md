...
- 使用者故事，一個故事一章，細節見各章的「需求」：
  ...
  - [在網頁上編輯設定並保留每個版本](#8-在網頁上編輯設定並保留每個版本settings)（`settings`）：✅
  - [換設定試跑而不影響正式分數](#9-換設定試跑而不影響正式分數dry-run)（`dry-run`）：〔規劃中〕
  - [追蹤同一筆職缺的評分變化](#10-追蹤同一筆職缺的評分變化history)（`history`）：✅，在展開列檢視〔規劃中〕
  - [看得出每個分數依據什麼評的](#11-看得出每個分數依據什麼評的basis)（`basis`）：記錄評分依據 ✅，在展開列顯示〔規劃中〕
...
- FR-score-salary：依 [§4.2.6](#426-薪資換算與計分) 換算月薪並計算薪資分數。
  ...
  - 薪資未知不會因此被淘汰。
- FR-score-ai：由 AI 依提示詞模板（[§4.2.8](#428-提示詞模板)），為三個 AI 維度各給 1–5 分或 `null`。
  - 每個維度都附上理由，另外提供總評。
...
偏好的格式是 YAML，鍵名用中文。所有欄位都是必填。缺少欄位或型別錯誤時拒絕並顯示原因，不補預設值（什麼時候檢查見 [§8.2.3](#823-檢查)）。
...
AI 維度實際收到哪些欄位，由提示詞模板用了哪些變數決定（見 [§4.2.8](#428-提示詞模板)），上列是預設模板的內容。

#### 4.2.5 AI 維度的錨點描述

這些描述是預設模板的內容，讓分數有一致的標準；使用者可以在模板中修改（見 [§8](#8-在網頁上編輯設定並保留每個版本settings)）。
...
- 「捨棄修改」讓編輯區回到目前設定。
- 儲存或套用前不動目前設定。
- 〔規劃中〕編輯區的內容給試跑用（見 [§9](#9-換設定試跑而不影響正式分數dry-run)）。
...
- 偏好照 [§4.2.2](#422-偏好的欄位字典) 驗證，有錯時拒絕並顯示原因，避免產生錯誤的評分。
- 經歷是自由格式，不檢查。
- 偏好與提示詞模板在這些時候檢查：
  - 儲存、套用前。
  - 〔規劃中〕試跑、送去評分前。
...
- 目前設定的偏好或經歷的內容和預設範例相同時（不論是第幾版）：
  - 標出「預設範例」。
  - 〔規劃中〕不能送去評分（見 [§4.2.13](#4213-送出前的確認)），試跑不擋。
...
- FR-basis-record：每筆評分紀錄記下評分依據，內容見 [§11.2.1](#1121-評分依據的內容)。
  - 被淘汰的職缺也記。
- FR-basis-show：〔規劃中〕展開列有評分依據，預設收合，規則見 [§11.2.2](#1122-顯示評分依據)。
  - 顯示的是展開列目前檢視的那筆評分紀錄的依據（見 [§10.2.2](#1022-所有評分紀錄)）。
- FR-basis-legacy：〔規劃中〕沒有記錄依據的舊評分，顯示「這筆評分沒有記錄依據」。
...
- NFR-privacy：API key 不進版控，版控中只提供範本 `.env.example`（見 [§13.2.5](#1325-ai-供應商與-api-key)）。
  - 偏好與經歷保存在職缺資料庫（見 [§8](#8-在網頁上編輯設定並保留每個版本settings)），跟著職缺資料庫不進版控。
...
- NFR-migration：資料庫結構改版時，保留評分紀錄與設定的版本。
...
- `評分依據`：評分時用的設定版本與職缺內容快照，見 [§11.2.1](#1121-評分依據的內容)
...
- 評分的職缺都來自職缺資料庫，不會有只有評分、沒有職缺資料的評分紀錄。
