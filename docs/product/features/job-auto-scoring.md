...
- 使用者故事，一個故事一章，細節見各章的「需求」：
  ...
  - [保留評分歷史並和手動評分並存](#11-保留評分歷史並和手動評分並存history)（`history`）：✅
...
調整提示詞的開發者也用單筆評分反覆修改，並加上 `--dry-run`（`--job-no ... --dry-run`，見 [§14.2.2](#1422-cli)）避免寫進正式的評分。
...
- `評分明細`：整份評分結果，格式依 [§14.2.1](#1421-評分結果格式)，要看各維度的分數與理由、淘汰原因時讀這裡。
...
- 資料庫只放正式評分：試跑（換偏好檔、經歷、模型或提示詞來比較）時要加 `--dry-run`，否則會寫進資料庫，成為這筆職缺最新的評分（見 [§9](#9-換設定試跑而不影響正式分數dry-run)）。
...
- 每筆評完就寫入資料庫。
  - 寫入是新增一筆，不覆寫任何紀錄；和這筆職缺最新一筆自動評分完全相同時不新增（見 [§11](#11-保留評分歷史並和手動評分並存history)）。
  - 和手動評分（見 [job-score-database §5](job-score-database.md#5-手動記錄評分manual)）並存，互不覆寫（見 [§11](#11-保留評分歷史並和手動評分並存history)）。
...
- Then：
  ...
  - 寫入的 `評分明細` 符合 [§14.2.1](#1421-評分結果格式)，淘汰、總分、評語與其中的值一致
  - 淘汰那筆的總分為 `null`
  - 評分失敗那筆的舊結果不變
- 通過條件：全部符合。
...
- 快取鍵不包含程式：
  ...
  - 修改 AI 回應的解讀方式後不會重問 AI。不提供清掉評分的方式（見 [§11](#11-保留評分歷史並和手動評分並存history)），要重問 AI 時改動快取鍵的來源，或等強制重新評分的參數（見 [TODO.md](../../../TODO.md#職缺自動評分範圍外)）。
- 被淘汰的職缺不呼叫 AI，不需要快取，每次都重新判斷，照 [§11.2.2](#1122-什麼時候新增一筆) 決定是否新增一筆。
- 快取不設期限。
...
- 沿用時照 [§11.2.2](#1122-什麼時候新增一筆) 決定是否新增一筆：
  - 新增的那筆，供應商、模型與沿用的那筆相同。
  - 與最新一筆完全相同時不新增，最新一筆的評分時間不變。
...
- 不帶 `評分明細` 與上次送給 AI 的內容：要看各維度的分數與理由時，以職缺代碼另外取出（見 [§7](#7-評分結果存進評分資料庫store)）。
...
- FR-history-source：每筆評分紀錄標示 `評分來源`，值依 [§11.2.1](#1121-評分來源)。
  - 既有的評分紀錄：有評分明細的視為 `auto`；沒有評分明細的是手動寫入的，視為 `manual`。
...
- 判斷能否沿用時，從這筆職缺的所有 `auto` 紀錄中找送給 AI 的內容相同的一筆，有多筆時用最新的一筆。

#### 11.2.3 多筆評分時的查詢

[§12](#12-同一筆職缺有多筆評分時查詢以代表的評分為準current) 完成前，查詢這樣處理同一筆職缺的多筆評分紀錄：

- 依分數列出（[job-score-database §4](job-score-database.md#4-依分數查詢評分query)）：每筆評分紀錄各列一次，同一筆職缺可能出現多次，看不出是手動還是自動評分。
- 以職缺代碼取出評分紀錄（[job-score-database §6](job-score-database.md#6-取出單筆的完整評分紀錄detail)）：取評分時間最新的一筆，不分手動或自動。
- 以職缺代碼取出評分明細（[§7](#7-評分結果存進評分資料庫store)）：取最新一筆 `auto` 紀錄的評分明細；沒有 `auto` 紀錄時表示查不到。
...
- Given：
  - 一個升級前的資料庫，已有一筆自動評分與一筆手動評分的紀錄
  - 一筆正常、一筆會被淘汰的職缺，AI 的回應以固定內容代替
- When：開啟資料庫後評整批，再手動評分正常那筆與升級前手動評分的那筆
- Then：
  - 升級前的自動評分為 `auto`，`評分明細` 與升級前的 `評分結果` 相同
  - 升級前的手動評分為 `manual`，再手動評分時被覆寫
...
- 整批結果檔（[§6.2.2](#622-結果檔)）與寫進評分資料庫的 `評分明細` 欄位（[§7.2.1](#721-寫入的評分紀錄)）都以它為基礎。
- 使用中文鍵名，與爬蟲的輸出一致。
