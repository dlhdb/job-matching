...
- 使用者故事，一個故事一章，細節見各章的「需求」：
  ...
  - [同一筆職缺有多筆評分時，查詢以代表的評分為準](#12-同一筆職缺有多筆評分時查詢以代表的評分為準current)（`current`）：✅
...
同一筆職缺有多筆評分紀錄時，查詢以代表的評分為準，見 [§12](#12-同一筆職缺有多筆評分時查詢以代表的評分為準current)。
...
- 依分數列出時的排序與篩選照 [job-score-database §4.2.1](job-score-database.md#421-列出評過分的職缺的結果)，以代表的評分計算。
  - 例如手動評分淘汰、較新的自動評分沒淘汰時，只列出沒被淘汰的職缺不含這筆。
