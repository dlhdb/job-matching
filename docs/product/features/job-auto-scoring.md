...
- 使用者故事，一個故事一章，細節見各章的「需求」：
  ...
  - [保留評分歷史並和手動評分並存](#10-保留評分歷史並和手動評分並存history)（`history`）：〔規劃中〕
  - [同一筆職缺有多筆評分時，查詢以代表的評分為準](#11-同一筆職缺有多筆評分時查詢以代表的評分為準current)（`current`）：〔規劃中〕
...
- 單筆輸出：評分結果的 JSON 印在終端機，欄位見 [§13.2.1](#1321-評分結果格式)。
...
- 整批與單筆的結果都寫進評分資料庫，寫入的欄位見 [§7.2.1](#721-寫入的評分紀錄)，查詢見 [job-score-database §4](job-score-database.md#4-依分數查詢評分query)。試跑的結果不寫入。

## 4. 評單筆職缺並看懂每個分數（score）

身為求職者，我想要依照寫成檔案的偏好與經歷替職缺評分，並看到每個維度的分數與理由，以便每次都用同一套標準，並知道總分是怎麼來的、判斷要不要相信它。

調整提示詞的開發者也用單筆評分反覆修改，並加上 `--dry-run`（`--job-no ... --dry-run`，見 [§13.2.2](#1322-cli)）避免覆寫正式的分數。
...
- Then：
  - 輸出符合 [§13.2.1](#1321-評分結果格式)，四個維度依序出現
...
- 結果檔要能直接用來挑職缺，所以每筆除了 [§13.2.1](#1321-評分結果格式) 的欄位，還有：
...
依分數列出評分屬 [job-score-database §4](job-score-database.md#4-依分數查詢評分query)，本功能不另外提供。
...
- `評分結果`：整份評分結果，格式依 [§13.2.1](#1321-評分結果格式)，要看各維度的分數與理由、淘汰原因時讀這裡。
  - 依分數列出時不帶這一項，以職缺代碼另外取出。
  - 〔規劃中〕改名為 `評分明細`（見 [§10](#10-保留評分歷史並和手動評分並存history)）
...
- 每筆評完就寫入資料庫。
  - 〔規劃中〕寫入改為新增一筆，不覆寫任何紀錄（見 [§10](#10-保留評分歷史並和手動評分並存history)）。
...
- Then：
  ...
  - 寫入的 `評分結果` 符合 [§13.2.1](#1321-評分結果格式)，淘汰、總分、評語與其中的值一致
  - 淘汰那筆的總分為 `null`
  - 評分失敗那筆的舊結果不變
- 通過條件：全部符合。
...
- 快取鍵不包含程式：
  ...
  - 修改 AI 回應的解讀方式後，要清掉已存的評分結果才會重問 AI。
    - 〔規劃中〕不再提供清掉評分的方式，要重問 AI 時改動快取鍵的來源，或等強制重新評分的參數（見 [§10](#10-保留評分歷史並和手動評分並存history)、[TODO.md](../../../TODO.md#職缺自動評分範圍外)）。
- 被淘汰的職缺不呼叫 AI，不需要快取，每次都重新判斷並覆寫。
  - 〔規劃中〕改為新增一筆，不覆寫（見 [§10](#10-保留評分歷史並和手動評分並存history)）。
- 快取不設期限。
...
- 沿用也照常覆寫該筆結果（〔規劃中〕改為新增一筆，與最新一筆完全相同時不新增，見 [§10](#10-保留評分歷史並和手動評分並存history)）：
  - 評分時間是最後一次寫入的時間。
  - 供應商、模型與上次相同。
...
- FR-history-append：自動評分每次寫入都新增一筆紀錄，不覆寫任何紀錄。
  - 與同一筆職缺最新一筆自動評分完全相同時不新增，判斷方式見 [§10.2.2](#1022-什麼時候新增一筆)。
...
- FR-history-coexist：手動評分與自動評分並存。
  ...
  - 自動評分不覆寫手動評分。
- FR-history-comment：被淘汰的職缺，`評語` 改為淘汰原因組成的文字，格式見 [§10.2.3](#1023-淘汰時的評語)。
  - 評分資料庫的 `評語` 改為必填（見 [job-score-database §8.2.1](job-score-database.md#821-評分紀錄契約)），淘汰時也要有值。
...
- FR-history-rename：`評分結果` 欄位改名為 `評分明細`，內容不變。
  ...
  - 評分結果格式（[§13.2.1](#1321-評分結果格式)）指單筆評分輸出的整份 JSON，名稱不變。
...
- 評分結果（[§13.2.1](#1321-評分結果格式)）與整批結果檔（[§6.2.2](#622-結果檔)）的 `評語` 用同一段文字。
...
- `評語`（str | null）：AI 產生的一到兩句總評，程式不修改
  - 被淘汰時為 `null`
  - 〔規劃中〕被淘汰時改為淘汰原因組成的評語，型態改為 str（見 [§10](#10-保留評分歷史並和手動評分並存history)）
