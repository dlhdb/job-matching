...
- 同一次執行內的失敗重試：失敗的職缺下次執行會再評（見 [§4.2.12](#4212-要評哪些職缺)），立即重試見 [TODO.md](../../../TODO.md#職缺自動評分範圍外)
- 平行呼叫 AI：整批維持循序執行
...
- Given：
  ...
  - 固定的擬真測試資料：偏好檔、經歷檔，以及 5 筆改寫自 104 的職缺，已匯入測試用的職缺資料庫，還沒有任何評分
- When：評所有還沒評分的職缺
- Then：
  - 正常結束
  - 第一筆有工作內容、而且沒被淘汰的職缺評分成功
  - 評分成功與被淘汰的職缺都寫進資料庫，評分失敗的職缺沒有寫入
  - 沒被淘汰的職缺：
    - 評分明細（見 [§4.2.10](#4210-寫入的評分紀錄)）符合 [§10.2.1](#1021-評分結果格式)，四個維度依序出現
    - 每個維度都有非空的理由，分數是 1–5 或 `null`
    - 總分在 0–100 之間，總分、評語與評分明細中的值一致
    - 供應商、模型為 `gemini`、`gemini-3.8-flash`
  - 被淘汰的職缺：總分、供應商、模型為 `null`，`評分明細` 的 `淘汰原因` 不是空清單
  - 印出供使用者閱讀的內容：
    - 摘要與前幾名的評語
    - 第一筆有工作內容、而且沒被淘汰的職缺的評分明細
    - 資料庫的路徑，跑完保留，使用者可以直接打開查看
- 通過條件：
  - 自動檢查全部符合
...
- When：
  - (a) AI 的回應以固定內容代替，執行 CLI
  - (b) 沒有設定 API key，職缺資料庫中只有會被淘汰的那筆
  - (c) 沒有設定 API key
  - (d) AI 的回應以固定內容代替，正常那筆 AI 呼叫失敗
- Then：
